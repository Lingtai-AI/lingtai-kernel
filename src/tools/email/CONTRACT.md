...
this one tool. The implementation lives in `src/tools/email/`; the code is the
source of truth.
...
- Notification surface reads/dismissals: use the `notification` tool
  (`src/tools/notification/CONTRACT.md`). The email tool only *publishes* its
...
- Code navigation only: read `src/tools/email/ANATOMY.md`.
...
The schema (`src/tools/email/schema.py`) accepts `action` from a fixed enum plus
...
| Claim | Source | Test |
|---|---|---|
| The email intrinsic registers exactly one `email` tool (no separate `mail` intrinsic) | `src/tools/email/__init__.py:boot` | `tests/test_layers_email.py::test_email_intrinsic_no_mail_intrinsic` |
| `send` routes through the mailman delivery thread | `src/tools/email/manager.py:_send` | `tests/test_layers_email.py::test_email_send_through_mailman` |
| A cc'd send writes exactly one `sent/` record | `src/tools/email/manager.py:_send` | `tests/test_layers_email.py::test_email_send_cc_one_sent_record` |
| Bodies over the 50k-char hard limit are refused at send time | `src/tools/email/manager.py:_send` (`EMAIL_BODY_CHAR_LIMIT`) | `tests/test_layers_email.py::test_email_send_rejects_body_over_hard_limit` |
| Identical consecutive sends are blocked as a loop | `src/tools/email/manager.py:_send` | `tests/test_layers_email.py::test_email_blocks_identical_consecutive_send` |
| `read` marks inbox mail read | `src/tools/email/manager.py:_read` / `primitives._mark_read` | `tests/test_layers_email.py::test_email_read_marks_as_read` |
| `dismiss` marks read without returning bodies and rerenders the digest | `src/tools/email/manager.py:_dismiss` | `tests/test_layers_email.py::test_email_dismiss_marks_read_and_returns_no_bodies`, `tests/test_layers_email.py::test_email_dismiss_rerenders_notification` |
| `archive` moves inbox mail into `archive/` | `src/tools/email/manager.py:_archive` | `tests/test_layers_email.py::test_email_archive_moves_to_archive` |
| `delete` refuses the `sent` folder | `src/tools/email/manager.py:_delete` | `tests/test_layers_email.py::test_email_delete_from_sent_rejected` |
| `search` compiles the query as a regex and rejects bad patterns | `src/tools/email/manager.py:_search` | `tests/test_layers_email.py::test_email_search_invalid_regex` |
| Scheduled/recurring sends are removed from the schema and not routed | `src/tools/email/schema.py` | `tests/test_layers_email.py::test_email_schedule_removed_from_schema`, `tests/test_layers_email.py::test_email_schedule_payload_is_not_routed` |
| Sender identity is carried on inbound mail and surfaced on read | `src/tools/email/manager.py:_inject_identity` | `tests/test_email_identity.py` |
| Abs-mode replies resolve via `_return_route`, guarding the `#145` ambiguous self-route | `src/tools/email/manager.py:_resolve_reply_target` | `tests/test_email_abs_reply_route.py` |
