...
related_files:
  ...
  - src/tools/daemon/DAEMON_CONTRACT.md
...
in the sibling `src/tools/daemon/DAEMON_CONTRACT.md` (contract_version 2); this
file does not restate them. The implementation lives in `src/tools/daemon/`; the
...
- Backend architecture, MCP completion signaling, selected-skills disclosure,
  or CLI-backend routing: read `src/tools/daemon/DAEMON_CONTRACT.md`.
- Code navigation only: read `src/tools/daemon/ANATOMY.md`.
- Independent peer agents that outlive the parent: use `avatar` (see
  `src/tools/avatar/CONTRACT.md`). An emanation's lifecycle is bounded by the
...
its process, materializes tools/skills, or signals completion — that is
...
| Claim | Source | Test |
|---|---|---|
| `handle` dispatches the five actions; unknown actions error | `src/tools/daemon/__init__.py` | `tests/test_daemon.py` (dispatch), `tests/test_daemon_check.py::test_check_unknown_id_returns_error` |
| Default `max_emanations` is 100 and the override reaches the manager | `src/tools/daemon/__init__.py` | `tests/test_daemon.py::test_daemon_default_max_emanations_is_100`, `::test_daemon_max_emanations_override_reaches_manager` |
| Backend schema enum matches the ordered alias contract | `src/tools/daemon/__init__.py` | `tests/test_daemon_backend_options.py::test_backend_schema_enum_matches_ordered_contract`, `::test_backend_metadata_consistency_keeps_hidden_legacy_claude` |
| `check` returns state + events, honors `last`/`truncate`, validates inputs | `src/tools/daemon/__init__.py` | `tests/test_daemon_check.py::test_check_running_emanation_returns_state_and_events`, `::test_check_respects_last_parameter`, `::test_check_truncate_limits_string_fields`, `::test_check_rejects_zero_or_negative_last` |
| `check` surfaces a terminal event for a done emanation and failure error | `src/tools/daemon/__init__.py` | `tests/test_daemon_check.py::test_check_includes_terminal_event_for_done_emanation`, `::test_check_includes_failure_error` |
| CLI-backend `ask` returns immediately and enforces its own timeout | `src/tools/daemon/__init__.py` | `tests/test_daemon.py::test_ask_codex_returns_immediately_when_subprocess_hangs`, `::test_ask_codex_silent_subprocess_enforces_timeout` |
| Token rows are written to both the daemon and parent ledgers, tagged | `src/tools/daemon/run_dir.py` | `tests/test_daemon_run_dir.py::test_append_tokens_writes_daemon_ledger`, `::test_append_tokens_writes_parent_ledger_tagged` |
| `reclaim` cancels running emanations; agent stop shuts the daemon down first | `src/tools/daemon/__init__.py` | `tests/test_lifecycle_daemon_shutdown.py::test_agent_stop_shuts_down_daemon_before_heartbeat_and_lock` |
