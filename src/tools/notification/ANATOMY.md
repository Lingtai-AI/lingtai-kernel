...
- `INTRINSICS["notification"]` (`src/tools/registry.py:52`, inside the `INTRINSICS` dict at `src/tools/registry.py:47` — the successor to the deleted kernel `intrinsics/__init__.py` `ALL_INTRINSICS`) → `BaseAgent._wire_intrinsics()` (`src/lingtai_kernel/base_agent/__init__.py:691`) binds `handle()` into every agent's tool surface. **Membership in `INTRINSICS` is the mandatory-include mechanism** — the wiring loop is unconditional, with no manifest gate, so this tool is always present like `system`.
- Delegates into the kernel-root `notifications.dismiss_channel` (`src/lingtai_kernel/notifications.py:560`). All #424 guards therefore hold through this tool by construction.
...
- **Parent:** `src/tools/` (see `src/tools/ANATOMY.md`).
