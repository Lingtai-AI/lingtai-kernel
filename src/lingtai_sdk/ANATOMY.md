...
- **→ `lingtai_kernel`** — capability `setup()` functions type against `lingtai_kernel.base_agent.BaseAgent`; this is the only allowed outward dependency direction.
...
- **← `lingtai.services.file_io` / `lingtai.services.file_io_sidecar`** — `sys.modules` aliases (module identity preserved) pointing at `services/file_io.py` / `file_io_sidecar.py` here.
...
- **Compatibility is shim-based, not copy-based:** old paths alias the SDK modules via `sys.modules`, preserving module identity so `monkeypatch.setattr(lingtai.services.file_io_sidecar, "_packaged_binary", ...)` reaches the resolver's real globals. Pinned by `tests/test_sdk_namespace_fileio.py`.
