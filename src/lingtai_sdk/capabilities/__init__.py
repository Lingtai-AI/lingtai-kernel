"""Composable agent capabilities — the registry seam.

This module owns the ``name → module-path`` registry, group expansion, the
``CORE_DEFAULTS`` floor, capability normalization, and ``setup_capability()`` —
the single seam every capability flows through.

It lives in ``lingtai_sdk`` (SDK-01). The historical import location
``lingtai.capabilities`` re-exports every public symbol from here, so existing
``from lingtai.capabilities import ...`` callers keep working unchanged.

Registry entries are *absolute* module paths. File-I/O tools resolve to
``lingtai_sdk.capabilities.file.*`` (SDK-02); the remaining always-on floor and
the optional multimodal capabilities still resolve to ``lingtai.core.*`` and
``lingtai.capabilities.*`` respectively — those move in later SDK slices.
"""
...
# All entries are absolute import paths so resolution does not depend on which
# package this registry is imported from. File-I/O tools (SDK-02) resolve into
# ``lingtai_sdk``; the rest still resolve into ``lingtai`` until later slices
# move them.
_BUILTIN: dict[str, str] = {
    # Always-on floor (lingtai.core) — not yet peeled
    ...
    # File-I/O tools — owned by the SDK (SDK-02)
    "read": "lingtai_sdk.capabilities.file.read",
    "write": "lingtai_sdk.capabilities.file.write",
    "edit": "lingtai_sdk.capabilities.file.edit",
    "glob": "lingtai_sdk.capabilities.file.glob",
    "grep": "lingtai_sdk.capabilities.file.grep",
    # Optional/multimodal capabilities (lingtai.capabilities) — not yet peeled
    "vision": "lingtai.capabilities.vision",
    "web_search": "lingtai.capabilities.web_search",
}
...
def get_all_providers() -> dict[str, dict]:
    ...
    _USER_FACING: dict[str, str] = {
        "file": "lingtai_sdk.capabilities.file.read",
        "bash": "lingtai.core.bash",
        "web_search": "lingtai.capabilities.web_search",
        "knowledge": "lingtai.core.knowledge",
        "skills": "lingtai.core.skills",
