...
def build_agent_config(manifest: dict[str, Any], *, max_rpm: int) -> AgentConfig:
    ...
    return AgentConfig(
        ...
        # Codex-family and Kimi Code providers own their omitted-thinking
        # semantics at the adapter (Codex: reasoning.effort "xhigh"; Kimi
        # Code: no KIMI_MODEL_THINKING_EFFORT environment variable at all), so
        # an omitted manifest value stays the "default" omission sentinel for
        # them instead of being promoted to the legacy cross-provider "high"
        # main-session default — which for Kimi would turn every existing agent
        # into an explicit effort request its always-thinking default model
        # rejects outright.
        thinking=llm.get(
            "thinking",
            THINKING_OMITTED
            if str(llm.get("provider") or "").lower()
            in (*THINKING_PROVIDERS, *KIMI_THINKING_PROVIDERS)
