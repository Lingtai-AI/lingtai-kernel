...
DESCRIPTION = (
    "Feishu/Lark messaging tool. Strict envelope: {action,input,reasoning,summarize?}; "
    "action, input, and reasoning are required, and each input branch is closed. "
    "MCP OWNERSHIP: the orchestrator/admin owns configuration; avatars (empty or "
    "non-admin) must not configure or reconfigure this MCP, and network wiring is "
    "propagated automatically. Actions: send (receive_id plus exactly one text/content), "
    "check, read (chat_id), reply (exact compound message_id plus text/content), "
    "react, search, edit/delete Bot-sent logical messages, contacts, accounts, "
    "read-only settings, and manual. Compound IDs must be passed verbatim; verify "
    "account, recipient, target, and content. Replies follow topics by default and "
    "a gone target never becomes a fresh send. send/reply support text, markdown, "
    "post, schema-2.0 card, media, shares, and stickers; edit excludes media. "
    "Every chunk is attempted once: partial results retain exact delivered IDs and "
    "must not be replayed blindly. Business card callbacks, local control cards, "
    "native progress cards, and resident Task Cards have distinct semantics. "
    "Failures expose error_code/retryable/retry_after_seconds; settings is SHOW-only."
)
