...
When the final current notification contains all required message content and its exact message_id, the agent SHOULD NOT call check/read/search merely to reread it: do not call check/read/search merely to reread that content or fetch an ID already shown here. A shortened alternate preview or older-history overflow does not require recovery if a complete current copy is available; recover only the specific missing content, not identical bytes.
