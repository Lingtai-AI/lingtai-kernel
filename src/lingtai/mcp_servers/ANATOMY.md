...
| `telegram/`, `imap/`, `feishu/`, `wechat/`, `whatsapp/`, `cloud_mail/` | Curated MCPs using `_skill.py` for their `action="manual"` payloads (`telegram/manager.py`, `imap/manager.py`, `feishu/manager.py`, `wechat/manager.py`, `whatsapp/manager.py`, `cloud_mail/manager.py`). Messaging MCPs with runtime account identity also delegate their identity envelope/path/write policy to `_identity.py`; Telegram notifications additionally package last-20 structured `recent_messages` / `latest_incoming` metadata (plus `referenced_messages` — the full reply target when the current message replies to one outside the last-20 window) for the kernel `_meta.notification_persistent.mcp.telegram` lane (the durable message text lives there, not in the ephemeral notification — Jason #6148); WhatsApp notifications package the same allowlisted routing keys (`platform`/`conversation_ref`/`message_ref`) plus bounded last-10 structured `recent_messages` / `latest_incoming` (text capped at 500 chars, allowlisted fields only — never raw Cloud API payload/response/webhook metadata) for the kernel `_meta.notification_persistent.mcp.whatsapp` snapshot lane (`whatsapp/manager.py` `_structured_message`/`_conversation_context`/`ingest_webhook`). |
