...
- **Task Card render: rows, heartbeat elapsed, fixed footer.** `_format_task_card_text` (`telegram/manager.py:1506`) renders the batched multi-row form via `_format_rows_task_card_text` (`telegram/manager.py:1543`): one line per tool call (`tool.action`, redacted reasoning, own one-decimal elapsed seconds via `_format_elapsed` at `telegram/manager.py:1586`, `✓` when done). Redaction runs on each row before any excerpt/trim, every row stays represented under length pressure (only excerpts shrink, bounded by `_TASK_CARD_TEXT_LIMIT`), and both the running and frozen renders carry the fixed `_TASK_CARD_FOOTER` (`telegram/manager.py:66`, "⚠️ Progress only — don't reply to this Task Card."). The kernel owns the batch/timer: BaseAgent's pre-dispatch hook builds one row per call and starts a 0.5s monotonic heartbeat (elapsed quantized to one decimal so frames advance 0.5 → 1.0 → 1.5), the result hook freezes the completed row, and the payload the reverse channel carries is the `rows` list (see `base_agent/__init__.py` and `src/lingtai/tools/mcp/ANATOMY.md`).
