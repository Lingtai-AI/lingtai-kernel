...
| `codex_account_source.py` | 484 | `NoCandidateError` + `AccountSource` protocol + `AccountCandidate` + `FixedAccountSource` + `WeightedAccountSource` — candidate selection, exclusion, static/dynamic weight arithmetic, safe no-candidate diagnostics |
...
- `NoCandidateError` (L115) — preserves the legacy base message when diagnostics are absent; otherwise `diagnostic_fields()` (L138) and `with_diagnostics()` (L145) carry only sanitizer-approved `no_candidate_*` / `codex_account_*` scalar fields, with compact log suffix formatting via `_format_no_candidate_diagnostics()` (L80).
- `AccountSource` (L169, `Protocol`) — `select(...)` (L177) returns one candidate or raises `NoCandidateError`; `quota_targets(...)` (L183) exposes non-excluded `(auth_ref, sha8)` identities for the adapter's quota-read seam.
- `FixedAccountSource` (L196) — always returns the same single account (used when any Codex spelling carries an explicit `codex_auth_path`, and for the empty-pool legacy fallback); `exclude` containing its identity raises diagnostic `NoCandidateError` (L216).
- `WeightedAccountSource` (L252) — pool-file weighted sampling. `_snapshot()` (L294) re-reads the pool file fresh for each unsnapshotted operation (`pool_size` L328, `quota_targets` L332, `select` L348 — no caching at construction, so a live pool-file edit is observed on the next call). `select()` (L348) excludes by `sha8` identity AND legacy resolved-path string, keeps the true positional index (`enumerate`) so duplicate/aliased entries anchor to the ACTUAL occurrence drawn rather than a re-derived first-match lookup, then does an unbiased cryptographic draw (`_uniform_float`, L479, via `secrets.randbits`). `_compute_raw_weights()` (L428): static (`configured_weight`) when `quota_left_snapshot is None`; dynamic (`configured_weight * quota_left_fraction`) otherwise, with a full-snapshot-completeness check — ANY eligible account's missing/non-comparable entry falls the WHOLE draw back to static (`_is_comparable_fraction`, L469).
