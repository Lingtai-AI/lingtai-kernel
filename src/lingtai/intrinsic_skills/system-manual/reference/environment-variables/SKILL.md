...
| `LINGTAI_FAKE_APP_SERVER_MODE` | Selects the fake Codex app-server quota scenario; default unset (normal quota). | Test-only scenario string (`exhausted`). | Read by the fake app server at invocation; rerun test after changing. Unrecognized value falls back to normal quota. | `tests/_fake_codex_app_server.py`; do not set in production. |
