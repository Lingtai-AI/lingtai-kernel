...
{
  "identity": {
    "address": "/Users/.../.lingtai/leaves-avatar",
    "agent_name": "leaves-avatar",
    "mail_address": "leaves-avatar"
  },
  "runtime": {
    "current_time": "2026-04-30T01:11:26-07:00",
    "started_at": "2026-04-30T07:54:15Z",
    "uptime_seconds": 1031.1,
    "stamina": 36000,
    "stamina_left": 34968.9,
    "state": "active"
  },
  "tokens": {
    "input_tokens": 2086261,
    "output_tokens": 22268,
    "thinking_tokens": 3191,
    "cached_tokens": 1983296,
    "total_tokens": 2111720,
    "api_calls": 23,
    "estimated": false,
    "context": {
      "system_tokens": 12989,
      "tools_tokens": 14704,
      "history_tokens": 84756,
      "total_tokens": 112449,
      "window_size": 200000,
      "usage_pct": 56.2,
      "fixed_tokens": 27693,
      "growing_tokens": 84756
    }
  }
}
```
...
| 字段 | 类型 | 来源 | 变率 | 说明 |
|------|------|------|------|------|
| `identity.address` | `str` | 实测 | 恒 | 工作目录绝对路径 |
| `identity.agent_name` | `str` | 实测 | 恒 | agent 名称 |
| `identity.mail_address` | `str` | 实测 | 恒 | 邮件路由地址 |
| `runtime.current_time` | `str` | 实测 | 每次 | ISO 8601（含时区偏移），写入时的当地时刻 |
| `runtime.started_at` | `str` | 实测 | 恒 | ISO 8601 UTC，最近一次启动时间 |
| `runtime.uptime_seconds` | `float` | 文档 | 每次 | 自启动以来的运行秒数 |
| `runtime.stamina` | `int` | 实测 | 恒 | 最大 stamina 秒数 |
| `runtime.stamina_left` | `float` | 文档 | 每次 | 剩余 stamina 秒数 |
| `runtime.state` | `str` | 实测 | 每次 | 生命周期态：`active` / `idle` / `asleep` / `stuck` / `suspended` |
| `tokens.input_tokens` | `int` | 实测 | 累增 | 累计输入 token 数 |
| `tokens.output_tokens` | `int` | 实测 | 累增 | 累计输出 token 数 |
| `tokens.thinking_tokens` | `int` | 实测 | 累增 | 累计思考 token 数 |
| `tokens.cached_tokens` | `int` | 实测 | 累增 | 累计缓存命中 token 数 |
| `tokens.total_tokens` | `int` | 实测 | 累增 | 累计总 token 数 |
| `tokens.api_calls` | `int` | 实测 | 累增 | 累计 API 调用次数 |
| `tokens.estimated` | `bool` | 文档 | 单向 | token 计数是否为估算（False→True 后不回退） |
| `tokens.context.system_tokens` | `int` | 文档 | 每次 | 系统 prompt token 数 |
| `tokens.context.tools_tokens` | `int` | 文档 | 每次 | 工具定义 token 数 |
| `tokens.context.history_tokens` | `int` | 文档 | 每次 | 对话历史 token 数 |
| `tokens.context.total_tokens` | `int` | 文档 | 每次 | 总已用 token 数 |
| `tokens.context.window_size` | `int` | 文档 | 每次 | 上下文窗口大小 |
| `tokens.context.usage_pct` | `float` | 文档 | 每次 | 使用百分比 = total / window * 100 |
| `tokens.context.fixed_tokens` | `int` | 实测 | 每次 | 固定部分 token 数 = system_tokens + tools_tokens |
| `tokens.context.growing_tokens` | `int` | 实测 | 每次 | 对话历史等增长部分的 token 数 |

> **变率**：「恒」= 启后不变，可作测试判据；「每次」= 每 turn 重写；「累增」= 只增不减；「单向」= 仅能从默认值变更为异常值（如 `estimated: False→True`），不回退。

> **来源说明**：「文档」= 本文件此前已记录；「实测」= 2026-04-30 实机运行 leaves-avatar 时采集，`base_agent._save_chat_history()` 实际写入。
