...
- **Private internal actions (kernel-driven reverse channel):** MCP tool handlers may expose private internal actions (e.g. ``_task_card_update`` in Telegram) that are NOT listed in the public ``SCHEMA`` action enum and are NOT discoverable by the LLM. The kernel invokes them directly through ``MCPClient.call_tool(tool_name, args)`` on a client reference obtained from ``agent._mcp_clients_by_tool`` (a stable ``tool_name → client`` mapping built at MCP registration time in ``agent.connect_mcp`` / ``agent.connect_mcp_http`` at ``src/lingtai/agent.py:933-938,990-995``). Private actions bypass ``ToolExecutor`` entirely, so they never recurse through the tool dispatch pipeline.
- **Pre-dispatch hook (``_on_tool_pre_dispatch_hook``):** ``BaseAgent`` exposes an optional pre-dispatch hook (``src/lingtai/kernel/base_agent/__init__.py:2230-2290``) that ``ToolExecutor`` calls just before each tool handler runs. The hook receives ``(tool_name, tool_args, tool_call_id)`` and is best-effort: exceptions are caught silently. For parallel tool execution (``_execute_parallel`` at ``src/lingtai/kernel/tool_executor.py:1362``), pre-dispatch hooks are invoked **serially in original tool_call order** before futures are submitted, ensuring deterministic entry ordering. Turn lifecycle cleanup for hook-owned state lives in ``_handle_request`` / ``_handle_tc_wake`` (``src/lingtai/kernel/base_agent/turn.py:1040-1044,1066-1251``) via ``try/finally`` blocks that call ``_teardown_telegram_task_card``.
