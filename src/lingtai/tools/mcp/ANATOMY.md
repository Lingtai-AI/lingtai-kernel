...
- **Pre-dispatch hook (``_on_tool_pre_dispatch_hook``):** ``BaseAgent`` exposes an optional pre-dispatch hook (``src/lingtai/kernel/base_agent/__init__.py:2239-2356``) that ``ToolExecutor`` calls just before each tool handler runs. The hook receives ``(tool_name, tool_args, tool_call_id)`` and is best-effort: it never blocks the real turn. Because the reverse-call MCP client reports tool-level failures as an error *dict* (and can also raise), the hook inspects the returned payload via the ``_task_card_result_error`` / ``_task_card_result_message_id`` helpers so an error result is never mistaken for a created card (no fake success), computes the ``message_id`` once, stays fail-open (a failed create simply retries lazily next tool call; a failed update keeps the existing card id), and is made observable through ``_log_task_card_reverse_failure`` (error-dict path) and ``_log_task_card_reverse_exception`` (raised path) — content-free warnings carrying only ``phase``/``tool`` plus the result ``status`` or the exception *class* name, and deliberately never the reasoning excerpt, chat id, account, card id, or provider error text. For parallel tool execution (``_execute_parallel`` at ``src/lingtai/kernel/tool_executor.py:1362``), pre-dispatch hooks are invoked **serially in original tool_call order** before futures are submitted, ensuring deterministic entry ordering. Turn lifecycle cleanup for hook-owned state lives in ``_handle_request`` / ``_handle_tc_wake`` (``src/lingtai/kernel/base_agent/turn.py:1040-1044,1066-1251``) via ``try/finally`` blocks that call ``_teardown_telegram_task_card``.
