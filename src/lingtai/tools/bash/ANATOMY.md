...
- `kernel/session_stats` — read-only presentation consumer of atomic `state.json` documents. Its background `async_work` snapshot maps safe lifecycle facts only; Shell still owns execution, poll/cancel, and all raw command/cwd/environment data (`src/lingtai/kernel/session_stats/__init__.py:554-633`).
...
- `bash/bash_policy.json` — packaged POSIX denylist policy, selected only by explicit `yolo: false` without a `policy_file` (the default is yolo). Denies destructive (`rm`, `rmdir`, `shred`, `dd`), privilege escalation (`sudo`, `su`, `doas`), permission changes (`chmod`, `chown`, `chgrp`), disk management (`mount`, `umount`, `mkfs`, `fdisk`), package managers (`apt`, `apt-get`, `yum`, `dnf`, `brew`), process control (`kill`, `killall`, `pkill`, `shutdown`, `reboot`, `systemctl`), network (`nc`, `ncat`), and code execution (`eval`, `exec`).
...
bash/__init__.py
  ...
  │
  └── setup(agent, policy_file, yolo=None) — forwards config; `_tool_family._bind` resolves policy, registers shell tool
```
...
- **Policy selection:** `_tool_family._bind` is the one resolver for direct setup, capability/preset kwargs, and the detached daemon binding: explicit `yolo=True` → allow-everything; else explicit `policy_file` → that file; else explicit `yolo=False` → packaged `bash/bash_policy.json` (`powershell_policy.json` for PowerShell); else (omitted) → yolo. `CORE_DEFAULTS["shell"]` is `{}`.
