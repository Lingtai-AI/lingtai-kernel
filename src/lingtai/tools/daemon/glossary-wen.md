...
- `tasks`：'emanate' 之任务列。每项以 `task` 为必填完整 daemon 系统指令，合目标、身份、约束、工具策、协作界与安危姿态；`prompt` 可选，仅 LingTai backend 用作第一条常规用户消息，省略或空白则精确为 `Begin the assigned daemon task.`。`system_prompt` 已除，无别名。
- `tasks.skills`：此 daemon 一务之可选 skill 上下文。字符串列；每项可为 skill 目录（内有 SKILL.md），亦可直指 SKILL.md。相对路径按父灵工作目录解之。daemon runtime 解析诸 skill frontmatter，渲为紧凑 YAML skill list 注入本次心令；以 `task` 明其何时/如何用之。
...
- `tasks.prompt`：可选。仅 LingTai backend 收为第一条常规用户消息；省略、空串或全空白则精确用 `Begin the assigned daemon task.`。外部 CLI backend 不受此字段，且会于创建 run-dir 前拒之。
