...
- **CLI resume `ask` is asynchronous; live detached LingTai `ask` is durably queued.** `_handle_ask_claude_interactive` / `_handle_ask_cli` / `_handle_ask_codex` spawn the resumed CLI subprocess on the calling thread (so subprocess-launch errors like missing CLI surface synchronously) but hand the stream-json/JSONL parse loop to a dedicated `ThreadPoolExecutor` (`_ask_pool`, sized to `manager_pool_size`). Terminal-resume `daemon(action="ask")` returns `{"status":"sent","async":true}` within milliseconds; progress lands as `cli_output` events + `last_output` and a typed follow-up notification. A live detached LingTai ask instead writes the shared RunDir inbox and returns `queued` plus its ID; the next checkpoint or native text-only boundary records and delivers it once. The old in-process `followup_buffer` branch remains compatibility/test-only, not production delivery evidence.
