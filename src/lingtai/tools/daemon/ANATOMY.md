...
daemon/__init__.py
  ...
  └── _publish_daemon_notification() — publishes one compact `.notification/system.json` event (id, terminal status, task summary, run dir, result/error path, bounded preview); terminal callers pass a stable idempotency key. Called directly by parent-process code paths (e.g. `_publish_followup_if_live`) that still run in this process; the detached supervisor's own terminal path uses its own equivalent in `supervisor_runtime.py`, not this method (`daemon/__init__.py:3762`)
