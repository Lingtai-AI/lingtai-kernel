...
`.notification/daemon.json` event — never as ordinary parent request text — and
...
daemon/__init__.py
  ...
  ├── _run_deepseek_emanation()     — DeepSeek Harness (`deepseek`) execution backend; runs one `--profile headless` task with a generated per-run Cordis patch, selected skill copies, native DSH MCP clients, run-private JSONL sessions, an explicit workspace, and LingTai-owned path/check acceptance receipts. On Windows it locates npm's PowerShell-visible `dsh.ps1`, resolves the adjacent official `@deepseek-ai/dsh/lib/bin.js`, and launches it with native Node; no WSL/POSIX shell is involved. User `--patch` overlays remain available, while LingTai appends its policy patch last. Session resume is not claimed, so `ask` is explicitly unsupported.
  ...
  └── _publish_daemon_notification() — publishes one compact `.notification/daemon.json` event (id, terminal status, task summary, run dir, result/error path, bounded preview) and stamps the channel's `data.daemon` batch state; terminal callers pass a stable idempotency key. Called directly by parent-process code paths (e.g. `_publish_followup_if_live`) that still run in this process; the detached supervisor's own terminal path uses its own equivalent in `supervisor_runtime.py`, not this method (`daemon/__init__.py:3762`)
...
- **Startup retries unpublished terminal notifications:** after stale-parent reaping, `DaemonManager.__init__` scans terminal run dirs and republishes only `done`/`failed`/`cancelled`/`timeout` records that explicitly carry `terminal_notified=false`. A stale `terminal_notification_claim` on a new-schema record is retryable; legacy records with `terminal_notified=true` or with the key absent are treated conservatively as already handled. The retry uses `daemon-terminal:<run_id>` as the event idempotency key, deduping a publish-before-receipt crash while the original event remains in the capped 20-event `daemon.json` window; after dismissal or eviction, recovery may safely republish because the durable receipt was never falsely recorded.
