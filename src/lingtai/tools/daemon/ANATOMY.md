...
daemon/__init__.py
  ...
  ├── _run_emanation()              — LingTai worker loop: sends `prompt`/default first, runs ToolExecutor/ToolCallGuard, performs same-run sole-call compact resets while retaining the exact call/result pair, and holds one per-run completion-recovery latch: a single nonempty text-only response with a missing/invalid current-run completion gets exactly one same-session `kind="completion_recovery"` user message (one continuation slot, durable `daemon_completion_recovery` event) before the unchanged `finish(done)` gate
