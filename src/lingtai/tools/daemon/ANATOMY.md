...
daemon/__init__.py
  ...
  ├── _handle_ask()                 — live native owners with the launch-time shared-inbox marker and supported active common-MCP CLI runs get direct durable inbox admission; unmarked version-skew native owners get only a legacy control-spool submission; terminal resumable CLI runs dispatch a separate resume owner
...
daemon/run_dir.py
  ...
  ├── enqueue_checkpoint_message()   — durably admits one bounded/redacted ID-bearing parent correction while live; optional deterministic control IDs make legacy spool replay idempotent
  ├── drain_parent_messages()          — atomically claims at most one bounded shared-inbox batch for the native text-only boundary, chunks oversized legacy strings, preserves the unclaimed tail, and records bounded delivery evidence
  ├── record_checkpoint()            — under the same RunDir transaction increments sequence, stores latest, arbitrates the shared inbox exactly once against native delivery, appends `daemon_checkpoint`, and touches heartbeat without changing terminal fields
...
- **Cooperative parent messages are nonterminal, bounded, and exactly once:** a live detached native LingTai owner advertises `native_parent_message_protocol=shared-inbox-v1` at launch before refreshed parent code may admit directly; supported active common-MCP CLI asks also enter the RunDir ID-bearing inbox only after durable admission. An unmarked but live native owner is treated as a surviving pre-upgrade process: the parent durably submits to its legacy control spool and does not claim a shared-inbox route/message ID. A native checkpoint or legal text-only boundary atomically claims at most one 20-row batch; oversized legacy strings are chunked to the admission-size bound and the unclaimed tail remains durable for later no-loss draining. Deterministic legacy control IDs and bounded lazy migration prevent duplicate/lost/invisible controls. `daemon(check)` projects pending IDs/counts plus bounded cumulative delivered IDs/total/route without message text. Unsupported active backends retain `busy`; terminal resumable CLI asks dispatch their separate resume owner; checkpoint delivery is not streaming chat, preemption, cancellation, or a terminal receipt.
...
- **CLI resume `ask` is asynchronous; live detached LingTai `ask` is capability-routed.** `_handle_ask_claude_interactive` / `_handle_ask_cli` / `_handle_ask_codex` spawn the resumed CLI subprocess on the calling thread (so subprocess-launch errors like missing CLI surface synchronously) but hand the stream-json/JSONL parse loop to a dedicated `ThreadPoolExecutor` (`_ask_pool`, sized to `manager_pool_size`). Terminal-resume `daemon(action="ask")` returns `{"status":"sent","async":true}` within milliseconds; progress lands as `cli_output` events + `last_output` and a typed follow-up notification. A marked/current live detached LingTai owner writes the shared RunDir inbox and returns `queued` plus its ID; the next checkpoint or native text-only boundary records and delivers it once. An unmarked/pre-upgrade live native owner instead accepts a durable legacy control-spool submission and returns only `{status:"sent",id}`; this is not a shared-inbox admission or delivery claim. The old in-process `followup_buffer` branch remains compatibility/test-only, not production delivery evidence.
...
- **Manual:** `daemon/manual/SKILL.md` — skill documentation for the LLM. It
  ...
  `cleanup/`, `forensics/`, and `dispatch-ledger/`, plus `cli-backends/`, whose own
