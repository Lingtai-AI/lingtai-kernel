...
daemon/run_dir.py
  ...
  ├── drain_parent_messages()          — atomically claims the shared inbox for the native text-only boundary, migrates legacy strings, and records bounded delivery evidence
...
- **Cooperative parent messages are nonterminal, bounded, and exactly once:** live detached native LingTai asks and supported common-MCP CLI asks enter one RunDir ID-bearing inbox only after durable admission. A native checkpoint or legal text-only boundary atomically claims the same rows; deterministic legacy control IDs and lazy migration of pre-upgrade strings prevent duplicate/lost/invisible controls. `daemon(check)` projects pending IDs/counts plus bounded cumulative delivered IDs/total/route without message text. Unsupported active backends retain `busy`, terminal resume paths remain unchanged, and checkpoint delivery is not streaming chat, preemption, cancellation, or a terminal receipt.
