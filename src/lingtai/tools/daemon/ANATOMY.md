...
- **Summary composition edge:** `_build_daemon_apriori_summarizer_fn` in
  ...
  the daemon logger preserves the raw result in the run-local
  `daemons/<run-id>/logs/events.jsonl` as `daemon_tool_result`, and the generated
  summary exposes that exact recovery locator to the worker.
