...
daemon/__init__.py
  ...
  ├── _handle_ask_codex()           — codex follow-up via `codex exec resume <codex_session_id> --json`. Symmetric with `_handle_ask_cli`: allocates a fresh live completion generation before spawn, or preserves the detached supervisor's claimed path; then dispatches to `_ask_pool` and returns immediately
  ├── _run_ask_codex_stream()       — background worker for the codex ask. Same JSONL parse as `_run_codex_emanation`; a fresh per-generation `daemon_common.finish(done)` receipt gates accepted follow-up progress before `daemon(check)` exposes it. Fresh validation enforces the finish payload's exact allowed keys and present-value types, while initial receipts remain identity-optional
  ...
  ├── _publish_daemon_notification() — publishes one compact `.notification/system.json` event (id, terminal status, task summary, run dir, result/error path, bounded preview); terminal callers pass a stable idempotency key, while live in-process follow-up (`ask`) completions publish one structured success/failure event; detached resume owners defer both outcomes to the supervisor's direct store path after the generation receipt (`daemon/__init__.py:3742-3854`). Detached follow-up publication is reconciled from the per-generation claim/receipt after store failure or a publish-before-receipt crash
...
daemon/run_dir.py
  ...
  ├── claim_followup_notification() / mark_followup_notification_published() — per-generation detached follow-up claim/receipt state stored in `resume-claims/resume-<generation>.json`; the generation-specific idempotency key is claimed before release, remains pending on sink failure or a receipt crash, and is cleared only after successful or idempotently observed publication
...
- **Detached follow-up notifications are durable per generation:** `run_resume_owner` claims `daemon-followup:<run_id>:<generation>` before releasing the generation, publishes success and failure through the direct notification-store path, and records the receipt only after the sink succeeds. A false/error result or crash between sink acceptance and receipt persistence leaves the claim pending; fresh-manager reconciliation retries the same key and idempotency recognition prevents duplicates, independently of prior events sharing the daemon ref id.
