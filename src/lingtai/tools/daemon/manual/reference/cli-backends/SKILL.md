...
| `deepseek` | Windows: `node <official-dsh-bin.js> --profile headless ... --patch <run-patch> <prompt>`; POSIX: `dsh --profile headless ...` | Not supported yet; `ask` returns an explicit unsupported-backend error | Official DeepSeek Harness execution backend (**developer preview**). LingTai generates a per-run Cordis patch for selected skills/MCP, private JSONL sessions, workspace policy, and independent path/check acceptance. The canonical backend id is `deepseek`; see the child page. |
