...
- **Pre-request hook convention** (`f46b346`) — every adapter `send()` / `send_stream()` checks `self.pre_request_hook` after committing the message to the canonical `ChatInterface` and before the API call. The kernel uses this to drain the involuntary tool-call inbox mid-turn. Two regimes: canonical-interface adapters (anthropic, openai-CC, codex-Responses, deepseek) get same-turn delivery for spliced pairs; server-state adapters (`OpenAIResponsesSession`, both Gemini sessions) get next-turn delivery because their wire payload is built from server-side state, not the canonical interface. Either way the agent-side drain (events.jsonl, chat_history.jsonl, .status.json) happens immediately. See kernel `llm/ANATOMY.md` for the ABC contract and root `ANATOMY.md` for the producer/consumer story.
- **Git history** — 16 commits. Key: Codex stateless path (`7e88f47`, `a4bf117`), context overflow recovery (`f65e395`), orphan tool_call guard (`8197fdc`), per-call HTTP timeout (`e279965`), pre-request hook for mid-turn tc_inbox drain (`f46b346`).
