...
- **Native Codex 1..N account lifecycle** — `CodexOpenAIAdapter._select_codex_account` owns quota-aware epoch selection, token refresh, exclusion state, and safe attribution for both fixed and weighted sources. Each `ChatInterface` gets one `_CodexAccountContext` with its own binding, safe selection, molt marker, exclusion chain, and authenticated client; the cached adapter is only the factory/source owner. `CodexResponsesSession.send_stream` performs the hard/explicit context reset before account binding, so a no-summary 100% forced rebuild gets exactly one fresh draw and the following ordinary request reuses it. Approved summarize/reconstruction boundaries redraw; technical transport/self-heal/turn-count resets do not. A complete per-account quota snapshot drives the fresh epoch's dynamic weights; incomplete quota data falls the whole draw back to static. Independently proven-zero accounts remain excluded for mixed snapshots, but an all-zero quota preflight that would erase every otherwise-available configured candidate is softened to static selection so the real request path adjudicates current usability. The bound account's safe realtime quota is refreshed on every real request when comparable, with unavailable data omitted rather than reported as zero. Only a truly empty pool may bind the legacy fallback; a non-empty exhausted pool fails closed for AED, with safe `NoCandidateError` category/count diagnostics rather than a bare opaque error.
  - **Account/continuation isolation:** when the selected account SHA changes, `_codex_refresh_account_for_request` resets the WebSocket epoch before rebinding (`openai/adapter.py:4288-4297`), closing authenticated transport and previous-response/compaction continuation state so no wire state crosses accounts.
  - **One retry owner:** only the final exception escaping Codex's built-in transport/self-heal paths is reported (`openai/adapter.py:4268-4275`). Structural `usage_limit_reached` adds that safe account identity to the adapter's exclusion set and clears the sticky binding (`openai/adapter.py:5401-5410`), then re-raises; the existing kernel AED loop owns the later session rebuild/replay. Success clears the current exclusion chain but never preselects the next account (`openai/adapter.py:5411-5413`).
  - **Partial output is terminal:** once a text delta was delivered, the escaping exception is marked `_lingtai_partial_stream` (`openai/adapter.py:5395-5400`); BaseAgent ends that turn rather than replaying already-visible text on another account.
  - **Safe selection metadata:** `codex_pool_selection` retains stable non-secret fields (`source_ref`, source index, pool size, weight, `auth_path_sha8`, model scope, quota/fallback when present) for response-usage and token-ledger attribution. Plaintext token contents never enter metadata; terminal no-candidate diagnostics are separately constrained to exact-allowlisted count/bool fields and fixed string enums, and no account health/cooldown state is persisted. Tests: `tests/test_codex_native_multiaccount.py`, `tests/test_codex_account_source.py`.
