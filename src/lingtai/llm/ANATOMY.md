...
- **Native Codex 1..N account lifecycle** — `CodexOpenAIAdapter._select_codex_account` (`openai/adapter.py:5095-5216`) owns quota-aware epoch selection, token refresh, exclusion state, and safe attribution for both fixed and weighted sources. `CodexResponsesSession.send_stream` calls `_codex_refresh_account_for_request` first (`openai/adapter.py:4253-4286`), but the adapter draws once for the current product context epoch and reuses that binding for ordinary requests. An approved summarize/reconstruction reset (`summarize_rebuild_only` or `summarize_delayed`) clears the binding; refresh/restart starts with a new adapter; a live molt is detected from the changed `molt_count`. Technical transport/self-heal/turn-count resets do not redraw. A complete per-account quota snapshot drives the fresh epoch's dynamic weights; incomplete quota data falls the whole draw back to static, while independently proven-zero accounts remain excluded. The bound account's safe realtime quota is refreshed on every real request when comparable, with unavailable data omitted rather than reported as zero. Only a truly empty pool may bind the legacy fallback; a non-empty exhausted pool fails closed for AED.
