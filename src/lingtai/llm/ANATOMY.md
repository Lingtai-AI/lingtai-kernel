...
| `base.py` | 227 | `LLMAdapter` ABC (4 abstract methods), `_GatedSession` proxy |
...
- **Gated sessions** — `_GatedSession` (`base.py:19`) proxies `send()` and `send_stream()` through `APICallGate.submit()`. The proxy is transparent in both directions: reads fall through to the inner session via `__getattr__` and writes forward to it via `__setattr__`/`__delattr__`, except the proxy's own `_inner`/`_gate` slots (`_PROXY_SLOTS`). Write-forwarding is load-bearing (#724): the inner reads `pre_request_hook`/`session_id`/`_agent_type`/`_tracked` on its own `self` from inside `send()`/`get_state()`, so a write that stayed on the proxy would never be seen — and since default `max_rpm=60` gates almost every session, that trap hit every default-configured process.
