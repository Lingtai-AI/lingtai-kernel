...
| `_codex_affinity_id()` | ~120–133 | Derive the epoch-stamped **current** 8-char affinity id: `sha256(f"{anchor}:epoch:{int(epoch)}").hexdigest()[:8]`. Same helper for both rotate triggers — the start/refresh build epoch and the 5-call-corruption rotate epoch (`time_fn()`). A different epoch (a rebuild or a rotate) yields a different id; sub-second jitter is ignored |
...
2. **5-call cache corruption** — the backend returns the SAME positive `cached_tokens` for `_CODEX_CACHE_AFFINITY_ROTATE_QUEUE_LEN` (= 5) requests in a row; the session re-derives a fresh current id (same helper, new epoch) and keeps using it for ALL subsequent requests (persistent, no revert).
...
- On the normal/root path the current id is `_codex_affinity_id(anchor, epoch) = sha256(f"{anchor}:epoch:{int(epoch)}").hexdigest()[:8]` (lowercase hex), where `anchor` is the per-agent durable identity (the resolved `init.json` path) and `epoch` is the adapter build time (injectable as `codex_epoch` / `time_fn`). It does **not** use the latest `api_call_id`, molt time, or generated UUIDs. Within one adapter instance it is stable across requests; a refresh/rebuild (new adapter, new epoch) changes it, and so does an 5-call corruption rotate — those are the only two triggers.
...
- **Token-ledger dump.** `CodexResponsesSession._usage_extra` copies the **actually-used** `session-id` / `thread-id` / `prompt_cache_key` for the request into `UsageMetadata.extra` as `codex_session_id` / `codex_thread_id` / `codex_prompt_cache_key`. Because of the normalization above these three are the **same value** (the current id, or the rotated id after an 5-call corruption) — the ledger never records mismatched affinity ids. `BaseAgent._save_chat_history()` merges all non-None usage extra fields into `logs/token_ledger.jsonl` (`base_agent/__init__.py:1654`), so the ids sit beside input/output/thinking/cached token counts. Surfacing the cache key makes an 5-call-corruption rotation (below) visible in the ledger. The values are short, non-secret derived affinity ids — no request body, messages, or OAuth secret ride along. A body-only/bare session contributes only the fields whose levers were actually sent.

### Codex 5-call cache-corruption rotate (`_record_cache_hit` / `_rotate_current_affinity`)

**Codex-only (Jason's final #406 semantics).** A *stalled/corrupted* cache slot is one where the backend keeps returning the **same** `cached_tokens` request after request — the cached prefix is no longer growing, so the current affinity id is buying nothing. `CodexResponsesSession` detects this and rotates the current id in place (the second of the two rotate triggers; the first is start/refresh):

- `_record_cache_hit(cached_tokens, model=...)` runs after each `response.completed`. It appends **only positive** `cached_tokens` (a `0` is a miss, not a stalled hit) to a rolling queue of length `_CODEX_CACHE_AFFINITY_ROTATE_QUEUE_LEN` (= 5). When the queue is full and every value is byte-identical (`len(set(...)) == 1`), it calls `_rotate_current_affinity` and clears the queue (a fresh run of five is required to re-fire).
...
- **`CodexResponsesSession._current_id` / `._cache_hit_queue`** — cache-affinity rotate state. `_current_id` is the current epoch-stamped affinity id used byte-identically for all three levers; `_cache_hit_queue` is a rolling window of the last 5 positive `cached_tokens`. When the window goes byte-identical, `_rotate_current_affinity` re-derives `_current_id` (and `_prompt_cache_key` / `_session_id` / `_thread_id`) from `_affinity_anchor` + a fresh `_time_fn()` epoch — persistent, no revert — and clears the window. `_affinity_anchor` (per-agent durable identity for re-derivation), `_event_sink` (rotate-event callable → `logs/events.jsonl`), and `_time_fn` (injectable clock) are set at construction.
