...
| `CodexResponsesSession` | 1575–1835 | Stateless Responses variant for ChatGPT-OAuth `/backend-api/codex`; sends stable `session-id` / `thread-id` REST cache-affinity headers (`_cache_affinity_headers`), passed down by default from the agent path + last ledgered LLM API call id; copies the resolved ids into `UsageMetadata.extra` for `token_ledger.jsonl` |
| `CodexOpenAIAdapter` | 1837–1966 | Adapter variant that builds `CodexResponsesSession`; overrides `_default_prompt_cache_key` → `lingtai-codex:{model}:v1`; resolves the cache-affinity header ids via `_resolve_codex_ids` (returns `(None, None)` only for a bare/test adapter with no per-agent identity passed down); passes the resolved ids to the session |
...
- **Default wiring (the normal path — not opt-in, not opt-out).** For a Codex agent, `service.build_provider_defaults_from_manifest_llm(llm, ..., working_dir=...)` injects the per-agent identity automatically: `codex_session_anchor = str((working_dir / "init.json").resolve())` (the agent path / durable identity anchor → `session-id = _codex_session_id(anchor)`) and `codex_thread_salt = <last ledgered API call id>` (→ `thread-id = _codex_thread_id(session-id, salt)`). So a normal Codex agent sends stable per-agent `session-id`/`thread-id` out of the box. The **last call id** is resolved by `service._latest_call_id(working_dir)`: it scans `logs/token_ledger.jsonl` from newest to oldest, preferring `source="main"` rows, and returns the most recent ledgered `api_call_id` (`api_<hex>`, assigned by `SessionManager.send`) → falling back to a stable `"birth"` salt when no ledgered call id exists. The thread salt no longer depends on molt time; `service._latest_molt_time` still exists but is no longer wired into the salt. A refresh that picks up a new ledgered last call id re-resolves the salt and rebuilds the service, rotating `thread-id` while `session-id` stays stable.
...
- **Token-ledger dump.** When a request actually sends cache-affinity headers, `CodexResponsesSession` copies the actual sent `session-id` / `thread-id` into `UsageMetadata.extra` as `codex_session_id` / `codex_thread_id`. `BaseAgent._save_chat_history()` merges safe usage extra fields into `logs/token_ledger.jsonl`, so the ids sit beside input/output/thinking/cached token counts and cache-hit data. No request body, secrets, or `prompt_cache_key` values are recorded. Bare/test sessions with no ids contribute no extra fields.
