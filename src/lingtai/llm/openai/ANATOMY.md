...
| `CodexOpenAIAdapter` | `adapter.py:4920` | The one Codex provider specialization shared by `codex`/`codex-pool` aliases. It creates one `_CodexAccountContext` per `ChatInterface`, owning that context’s binding, safe selection, no-candidate diagnostic counts, molt marker, exclusion chain, authenticated client, and native request headers; the adapter remains the shared source/factory owner. It also owns cache/session ids, installation id, endpoint/service-tier settings, and Codex’s explicit default `reasoning.effort = "xhigh"`; no pool-specific session or retry adapter exists. |
...
6. After success: record the assistant response into the interface and recompute
   ...
   next turn can strict-prefix-match and stay incremental; the adapter clears the current exclusion chain without preselecting the next credential (`adapter.py:5411-5413`).

Pre-request account-selection failures are deterministic: `_select_codex_account()` enriches `NoCandidateError` with safe count diagnostics from `_codex_no_candidate_diagnostics()` (`adapter.py:5156-5191`, `adapter.py:5228-5363`) and re-raises without provider fallback unless the configured pool snapshot is truly empty. Complete zero-quota preflight remains a hard exclusion for mixed snapshots, but when it would erase every otherwise-available configured candidate, the adapter softens it to static selection and lets the provider path adjudicate current usability (`adapter.py:5305-5330`). Only the final exception escaping all built-in transport/self-heal paths reaches the adapter account-error callback (`adapter.py:4268-4275`). A structural `usage_limit_reached` excludes that safe account identity for the existing AED rebuild (`adapter.py:5419-5434`); if any text delta was already delivered, the exception is marked `_lingtai_partial_stream` so BaseAgent terminates rather than replaying visible output (`adapter.py:5421-5427`).
