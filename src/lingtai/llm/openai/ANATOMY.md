...
| `responses_reasoning_construction()` / `chat_reasoning_construction()` | `adapter.py:957-1045` | Byte-identical route-local reasoning primitives returning one immutable `ReasoningConstruction` per construction; each wire route (generic/custom Responses via `_responses_route_id`, Chat via `_chat_route_id`, Codex's `default→xhigh` override, DeepSeek/MiMo/Zhipu/OpenRouter wrappers) consumes that one result for both the actual kwargs and the recorded emission evidence |
