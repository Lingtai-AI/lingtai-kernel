...
| `CodexResponsesSession` | ~1571–1940 | Stateless Responses variant for ChatGPT-OAuth `/backend-api/codex`; sends stable `session-id` / `thread-id` REST cache-affinity headers (`_cache_affinity_headers`), passed down by default from the agent path (one 8-char hash, byte-identical for both); copies the actually-used `session-id` / `thread-id` / `prompt_cache_key` into `UsageMetadata.extra` (`_usage_extra`) for `token_ledger.jsonl`. Holds a rolling 5-deep cache-hit queue (`_record_cache_hit`); on 5 byte-identical positive `cached_tokens` it arms a one-shot temporary affinity id (`_arm_temp_affinity` / `_effective_affinity`) and emits a `codex_cache_stall_temp_key` event |
...
**Codex-only (issue #378).** For the normal/root main Codex session the three cache-affinity values are **byte-identical and stable**:

```
session-id == thread-id == prompt_cache_key == <8-char agent-path hash>
```

The REST `/backend-api/codex/responses` endpoint does **not** accept `previous_response_id` (`Unsupported parameter`), so the cache-affinity lever — like the official Codex client — is stable HTTP headers (`session-id` / `thread-id`), sent via the SDK's per-request `extra_headers` (never request-body fields), plus the request-body `prompt_cache_key`. All three carry the same value.

- The shared value is `_codex_session_id(anchor) = sha256(anchor).hexdigest()[:8]` (lowercase hex). It does **not** use the latest `api_call_id`, molt time, or generated UUIDs. Ordinary LLM calls, `api_call_id` rotation, refresh/rebuild, molt, and clear (same agent path) all leave it unchanged — only a different agent path changes it. (If the backend later proves to require UUIDs, that is a separate change; the current contract is the 8-char hash.)
...
- **These ids MUST be per-agent.** The value must NOT derive from the model-only `prompt_cache_key` form (`lingtai-codex:{model}:v1`) — every agent on a model shares that string, which would collapse all of them onto one session/thread. The adapter has no per-agent identity of its own, so the host wiring passes the agent path down by default (see below).
