...
| `OpenAIResponsesSession` | `adapter.py` | Responses API session. Official OpenAI mode is server-stateful via `previous_response_id`; custom/OpenAI-compatible mode is restart-safe canonical replay (`stateless_replay=True`). Its default HTTP transport sends full replay every turn; explicit Responses WebSocket v2 keeps one connection and advances only from a proven canonical prefix using delta plus `previous_response_id`, while durable history remains the full canonical interface. |
| `OpenAIAdapter` | `adapter.py` | `LLMAdapter` implementation; dispatches to Completions or Responses path; receives injected `compact_threshold`; derives the default `prompt_cache_key`; carries `_responses_stateless_replay` and the validated custom `responses_transport` into Responses sessions. |
...
| `CodexOpenAIAdapter` | `adapter.py:4892` | The one Codex provider specialization shared by `codex`/`codex-pool` aliases. It creates one `_CodexAccountContext` per `ChatInterface`, owning that context’s binding, safe selection, molt marker, exclusion chain, authenticated client, and native request headers; the adapter remains the shared source/factory owner. If selection ends in `NoCandidateError`, it attaches only allowlisted pool/exclusion/quota-scan counts and booleans for the kernel terminal event; this is observation-only and does not change selection or AED. It also owns cache/session ids, installation id, endpoint/service-tier settings, and Codex’s explicit default `reasoning.effort = "xhigh"`; no pool-specific session or retry adapter exists. |
...
- **OpenAI clients** — imports `httpx` for timeout construction and `openai` for API calls. Responses WebSocket v2 requires `openai[realtime]>=2.22.0`, declared directly in `pyproject.toml`; the extra supplies the SDK's `websockets` runtime so a local missing dependency cannot be misreported as an unsupported upstream.
...
1. **Responses API** (`_create_responses_session`) — when canonical `wire_api="responses"`, or when `wire_api="auto"` and legacy `use_responses=True` AND (`base_url` is None OR `force_responses=True`). Builds `OpenAIResponsesSession`, threading the adapter's `_responses_stateless_replay` into its `stateless_replay` kwarg — so a custom/OpenAI-compatible Responses adapter builds a stateless-replay session while official OpenAI stays stateful. `responses_transport="websocket"` is accepted only with explicit Responses plus stateless replay; absence/`http` preserves the existing HTTP path.
...
Three modes share one session class:
...
2. **Custom/stateless HTTP (default)** — `_snapshot_interface` captures the pre-stage snapshot before `_stage_input`, `_request_input` records string/tool-result input (or leaves `send(None)` pre-staged entries alone), enforces tool pairing, and serializes full canonical history through `to_responses_input`; no `previous_response_id` is sent. On success `_record_assistant_response` persists reasoning/text/tool calls plus usage, including cached tokens, into canonical history. On transport/stream/enforce/serialization/parse/finalize/callback/record failure, `_rollback_staged` restores the pre-send canonical snapshot in place while preserving the recovery lookup.
3. **Custom/stateless WebSocket v2 (explicit)** — uses the same staging, rollback, canonical recording, and durable-history rules as custom HTTP. The first request sends full replay; after success `_remote_prefix` and `_response_id` form an in-memory continuation basis. A later request sends only the strict-additive canonical delta plus `previous_response_id`; any prefix mutation starts a new full chain. This server state is never exposed as a durable resume id.

### Custom Responses WebSocket v2 transport

The manifest option exists only for `provider="custom"`, `api_compat="openai"`, and explicit `wire_api="responses"`; `responses_transport` is omitted/`http` by default and `websocket` is the sole opt-in value. The custom factory is the only registry path that forwards it. There is no HTTP fallback after WebSocket selection.

Connection setup uses the SDK's persistent `responses.connect()` with the v2 beta header and disabled protocol ping. Its `response.create` frame omits the HTTP-only `stream` option, as required by the WebSocket v2 protocol. HTTP handshake rejection statuses 400/403/404/405/406/410/426/501 raise `ResponsesWebSocketUnsupportedError` naming only the configured host and telling the user to select HTTP. 401 and non-status network failures raise `ResponsesWebSocketRequestError` instead, so bad credentials or transient network faults are not mislabeled as unsupported; a missing local SDK/runtime raises the separate `ResponsesWebSocketConfigurationError`. Provider error events accept both SDK objects and mappings, and bounded single-line diagnostics retain status/code/type/param/message plus WebSocket close code/reason without serializing requests, headers, or exception representations.

Retry ownership is bounded: a request with no continuation id may reconnect and resend once after a transport failure; a request carrying an established `previous_response_id` is never automatically resent. If the server explicitly reports that a continuation id is missing/expired, the session clears that one chain and rebuilds it once from full canonical history. A partial streamed text failure is marked `_lingtai_partial_stream` so BaseAgent ends the turn instead of replaying already-visible output. `reset()` closes the socket and clears only remote chain state, preserving canonical history for the next full replay. Tests: `tests/test_responses_websocket_v2.py`.
...
Before staging input, `send_stream` resolves the context boundary and then calls `_codex_refresh_account_for_request()` (`adapter.py:4253-4286`). Host-wired sessions therefore consume one AccountSource draw at the first real request of an approved context epoch, then reuse that account for ordinary requests while refreshing only its current quota metadata. A no-summary 100% forced rebuild is a true rebuild: it clears the owning context before that request, draws exactly once, and the next ordinary request reuses the new binding. Explicit summarize/reconstruction resets redraw; refresh/restart starts with a new adapter and molt is detected from the live `molt_count`. Technical transport/self-heal/turn-count resets preserve the binding. Every session owns its authenticated client and account headers, so interleaved chats cannot race credentials or clear one another’s epoch.
...
- **`OpenAIResponsesSession._response_id` / `_stateless_replay`** — in official/stateful mode `_response_id` is the server-side chain pointer and `session_resume_id`. Custom/stateless HTTP does not advance it. Custom WebSocket v2 uses it only with `_remote_prefix` as an in-memory strict-additive chain pointer; both custom transports still return no `session_resume_id`, and `get_history()` returns full canonical `ChatInterface` history for durable restart.
...
- **Responses streaming** — HTTP and custom WebSocket v2 consume the same Responses event families: `response.reasoning_summary_text.delta/done` (summary thoughts only), `response.output_text.delta`, `response.function_call_arguments.delta/done`, `response.output_item.added/done`, and `response.completed`. Custom/stateless mode snapshots before staging, records the finalized assistant turn, and restores the pre-send snapshot on enforce, serialization, stream-open, iteration, callback, finalize, or record failure. WebSocket additionally marks an exception after any delivered text delta as a terminal partial stream.
