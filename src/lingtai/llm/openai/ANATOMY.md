...
| `CodexResponsesSession` | ~1594–1965 | Stateless Responses variant for ChatGPT-OAuth `/backend-api/codex`. **Normalizes** its three affinity inputs at construction into ONE effective id (`adapter.py:1642-1651`, priority `prompt_cache_key` > `session_id` > `thread_id`) so `prompt_cache_key == session-id == thread-id` always (Jason's follow-up to #378). Sends `session-id` / `thread-id` REST headers (`_cache_affinity_headers`) only when a per-agent identity was supplied (a lone cache key stays body-only — header carve-out). Copies the actually-used (single) value into `UsageMetadata.extra` (`_usage_extra`) for `token_ledger.jsonl`. Holds a rolling 5-deep cache-hit queue (`_record_cache_hit`); on 5 byte-identical positive `cached_tokens` it arms a one-shot temporary affinity id (`_arm_temp_affinity` / `_effective_affinity`) — itself one value for all three — and emits a `codex_cache_stall_temp_key` event |
| `CodexOpenAIAdapter` | ~1945–2090 | Adapter variant that builds `CodexResponsesSession`; resolves the per-agent cache-affinity value from the anchor (or explicit `codex_session_id`); `_resolve_codex_ids` returns `(id, id)` — thread tracks session — and `(None, None)` only for a bare/test adapter; `_default_prompt_cache_key` returns the SAME per-agent value (falls back to `lingtai-codex:{model}:v1` only on the bare/no-anchor path); `_build_codex_event_sink` derives `logs/events.jsonl` (sibling of the `init.json` anchor) and passes an `event_sink` into the session (`None` on the bare/test path) |
...
| `_codex_temp_affinity_id()` | ~112–125 | Derive a short, log-safe **temporary** 8-char affinity id from the trigger event time (to the second): `sha256(f"codex-cache-stall:{int(ts)}").hexdigest()[:8]`. Used for the one-shot cache-stall swap (all three levers carry it for that one request) |
...
**Codex-only (issue #378, normalized in Jason's follow-up).** The three cache-affinity values are now a **session-enforced invariant** — never independent:

```
prompt_cache_key == session-id == thread-id == <effective affinity id>
```

`CodexResponsesSession.__init__` (`adapter.py:1642-1651`) **normalizes** whatever candidates it receives (`prompt_cache_key`, `session_id`, `thread_id`) into one *effective* id and uses it byte-identically for all three. Priority is `prompt_cache_key` > `session_id` > `thread_id` (the explicit request-body cache-affinity key wins). This closes the prior leak where an explicit `prompt_cache_key` override or a directly-constructed session could send three different values.
...
- On the normal/root path the effective id is `_codex_session_id(anchor) = sha256(anchor).hexdigest()[:8]` (lowercase hex). It does **not** use the latest `api_call_id`, molt time, or generated UUIDs. Ordinary LLM calls, `api_call_id` rotation, refresh/rebuild, molt, and clear (same agent path) all leave it unchanged — only a different agent path changes it.
...
- **Default wiring (the normal path — not opt-in, not opt-out).** For a Codex agent, `service.build_provider_defaults_from_manifest_llm(llm, ..., working_dir=...)` injects `codex_session_anchor = str((working_dir / "init.json").resolve())` (the agent path / durable identity anchor). The adapter then derives the 8-char hash from it and uses it for all three values via `_resolve_codex_ids` (returns `(id, id)` — the thread tracks the session id exactly) and `_default_prompt_cache_key` (returns the same `id`). The default wiring does **not** read the token ledger or molt time, so the same `working_dir` yields the same defaults regardless of ledger contents.
...
- **Token-ledger dump.** `CodexResponsesSession._usage_extra` copies the **actually-used** `session-id` / `thread-id` / `prompt_cache_key` for the request into `UsageMetadata.extra` as `codex_session_id` / `codex_thread_id` / `codex_prompt_cache_key`. Because of the normalization above these three are now the **same value** (the effective id, or the one-shot temp id on a swap request) — the ledger never records mismatched affinity ids. `BaseAgent._save_chat_history()` merges all non-None usage extra fields into `logs/token_ledger.jsonl` (`base_agent/__init__.py:1654`), so the ids sit beside input/output/thinking/cached token counts. Surfacing the cache key makes a cache-stall temporary-key swap (below) visible in the ledger, not just the stable id. The values are short, non-secret derived affinity ids — no request body, messages, or OAuth secret ride along. A body-only/bare session contributes only the fields whose levers were actually sent.

### Codex cache-stall temporary-key protection (`_record_cache_hit` / `_effective_affinity`)

**Codex-only (Jason's follow-up to #378).** A *stalled* cache slot is one where the backend keeps returning the **same** `cached_tokens` request after request — the cached prefix is no longer growing, so the stable affinity id is buying nothing. `CodexResponsesSession` detects this and breaks the stall with a one-shot temporary affinity id:

- `_record_cache_hit(cached_tokens, model=...)` runs after each `response.completed`. It appends **only positive** `cached_tokens` (a `0` is a miss, not a stalled hit) to a rolling queue of length `_CODEX_CACHE_STALL_QUEUE_LEN` (= 5). When the queue is full and every value is byte-identical (`len(set(...)) == 1`), it calls `_arm_temp_affinity` and clears the queue (a fresh run of five is required to re-fire).
- `_arm_temp_affinity` computes a temporary id from the **trigger event time to the second**: `_codex_temp_affinity_id(ts) = sha256(f"codex-cache-stall:{int(ts)}").hexdigest()[:8]` (short, log-safe, deterministic per trigger second). It is stored in `_pending_temp_id`. The clock is injectable (`time_fn`, defaults to `time.time`).
- `_effective_affinity()` is consulted when building each request. Normally it returns the stable `(prompt_cache_key, headers)`. If `_pending_temp_id` is set it **consumes it once** — the SAME temporary value is used byte-identically for `prompt_cache_key` / `session-id` / `thread-id` on exactly that request, then it reverts to the stable id. So the temp key is **one-shot**, not a permanent drift; this matches the requirement that it is *temporary*. A bare/test session with no stable ids stays header-free even mid-swap (the temp header is emitted only if its stable counterpart would have been).
- On a swap the session emits a `codex_cache_stall_temp_key` event via an injected `event_sink`. `CodexOpenAIAdapter._build_codex_event_sink` wires the sink to the agent's `logs/events.jsonl` (a sibling of the `init.json` anchor; `None` on the bare/test path, so nothing is written). The event carries only safe metadata: `type`, `ts`, `provider`, `model`, `had_stable_id` (bool), `temporary_id_hash`, `recent_cached_values` (the 5 stalled numbers), and a human `reason`. It deliberately does **not** include the stable id itself, any prompt/message body, token totals, or OAuth secret. Sink/write failures are swallowed (`logger.warning`) — observability never breaks a turn.
...
- **`CodexResponsesSession._cache_hit_queue` / `._pending_temp_id`** — cache-stall detector state. `_cache_hit_queue` is a rolling window of the last 5 positive `cached_tokens`; `_pending_temp_id` holds a one-shot temporary affinity id armed when the window goes byte-identical, consumed (reverting to stable) on the next request. `_event_sink` (swap-event callable → `logs/events.jsonl`) and `_time_fn` (injectable clock) are set at construction.
