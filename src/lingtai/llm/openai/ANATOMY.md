...
Pre-request account-selection failures are deterministic: `_select_codex_account()` enriches `NoCandidateError` with safe count diagnostics from `_codex_no_candidate_diagnostics()` (`adapter.py:5156-5191`, `adapter.py:5317-5338`) and re-raises without provider fallback unless the configured pool snapshot is truly empty. Only the final exception escaping all built-in transport/self-heal paths reaches the adapter account-error callback (`adapter.py:4268-4275`). A structural `usage_limit_reached` excludes that safe account identity for the existing AED rebuild (`adapter.py:5401-5410`); if any text delta was already delivered, the exception is marked `_lingtai_partial_stream` so BaseAgent terminates rather than replaying visible output (`adapter.py:5395-5400`).
