...
Experimental clean-room LLM provider that drives the public `claude_agent_sdk`
...
- **Single-turn generator** — `_build_options` sets `allowed_tools=[]`, `max_turns=1`. Each `send()` produces exactly one assistant text turn.
