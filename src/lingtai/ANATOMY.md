...
- **AgentConfig hydration:** `_setup_from_init` rebuilds runtime config via `build_agent_config`, overlaying explicit init.json values onto `lingtai.kernel.config.AgentConfig` defaults. `manifest.cache_miss_budget` overlays `AgentConfig.cache_miss_budget` (default 1,000,000). Legacy `max_turns` and `molt_*` manifest values remain deliberately ignored. `manifest.llm.thinking` hydrates verbatim when present for Codex-family providers and explicit custom OpenAI-compatible Responses; `llm_supports_thinking` is the shared scope rule. Custom Responses retains `none`/`minimal`/`low`/`medium`/`high`/`xhigh`. Codex explicit values are exact-model contract values. When omitted, Codex-family providers (`THINKING_PROVIDERS`) keep the `"default"` sentinel so the injected registered-route contract constructs explicit `reasoning.effort = "xhigh"` with source `lingtai_codex_default`; custom Responses and all other providers keep the legacy cross-provider `"high"` main-session default. Kernel-side `AgentConfig` construction resolves a constructor-omitted `thinking` to the same per-provider-scope values in `__post_init__` (`lingtai/kernel/config.py`, identity-compared unique sentinel — explicit strings are never reinterpreted as omission) and keeps a private constructor-omission provenance fact; `SessionManager._session_thinking()` resolves the effective route as `config.provider or llm_service.provider` at its single create/rebuild seam, so both direct `AgentConfig(provider=<codex-family>)` construction and a plain `AgentConfig()` with an injected native Codex service reach the same contract-owned omission path as wrapper hydration.
