...
**`cli.py`**: `_persist_venv_path` :22 · `load_init` :62 · `build_agent` :125 · `run` :259 · `_handle_log_command` :311 · `_handle_maintenance_command` :356 · `main` :401
...
| Path | When | What |
|---|---|---|
| `<workdir>/init.json` | `_activate_preset` :988, `cli._persist_venv_path` :22 (from `cli.run` :259), `_read_init` :906 | Preset swap (atomic write); venv_path writeback (patches only `venv_path` onto the raw on-disk file — never the resolved in-memory dict, #728); boot/refresh cleanup of deprecated top-level fields and archive-preserving init migrations |
