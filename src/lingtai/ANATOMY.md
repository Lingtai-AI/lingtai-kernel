...
**Agent → BaseAgent:** Three-layer hierarchy: `BaseAgent` (kernel) → `Agent` (capabilities) → `CustomAgent` (domain). Agent adds capability registration, MCP auto-loading, preset swap, full init.json reconstruct, and composes `PosixJsonlEventJournalAdapter` for callers that did not supply an event journal (`agent.py:115-141`).
