...
| `<workdir>/settings/psyche.json` | `tools/psyche/settings.py` via `Agent._setup_from_init` / `_reload_prompt_sections` | Optional strict Psyche-owned v1 prompt document. Missing defaults all six owner values; any present invalid/non-regular/raced document aborts reconstruction. Refresh validates it before live teardown. Agent never writes or migrates it. |
