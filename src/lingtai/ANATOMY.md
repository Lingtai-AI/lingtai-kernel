...
**Agent → BaseAgent:** Three-layer hierarchy: `BaseAgent` (kernel) → `Agent` (capabilities) → `CustomAgent` (domain). Agent adds capability registration, MCP auto-loading, preset swap, full init.json reconstruct, and composes `PosixJsonlEventJournalAdapter` plus `PosixNotificationStoreAdapter` for callers that did not supply those dependencies (`agent.py:115-151`).
