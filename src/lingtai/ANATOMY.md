...
| `adapters/acp/` | Governed ACP v1 driving Adapter: strict framing, one session/active prompt, canonical execution workspace, atomic session stdio MCP lease in the separate stdio host, baseline content translation, cancellation, and teardown. Its opt-in resident Unix socket attaches to an existing `run` Agent with session MCP disabled; see its Anatomy/Contract. |
...
| `cli.py` | `lingtai-agent run <dir> [--acp-socket]` / `acp-socket-path <dir>` / `lingtai-agent acp --agent-dir <dir>` / `lingtai-agent acp --profile puffo-v0\|puffo-v1 --runtime-id <id>` / `lingtai-agent puffo-v0 provision\|revoke` / `lingtai-agent project create ...` / `lingtai-agent check-caps` / `lingtai-agent log ...` / `lingtai-agent maintenance cleanup <target>` entry points; the `run` composition root first refuses any `.change-name-incomplete` marker shape before cleanup/init/Agent construction, optionally starts the local ACP socket after Agent start, and performs a post-stop hard exit only when existing worker-poison state would otherwise keep the old process alive and block the refresh watcher |
...
| `venv_resolve.py` | Python venv resolution — explicit `init.json` venv → global runtime → auto-create with platform-aware interpreter selection, plus kernel-owned `.lingtai-env.json` marker check/stamp semantics for TUI and kernel callers; ACP supplies its active virtualenv as the default. Bootstrap child diagnostics use stderr and unavailable published version pins fail without an unpinned retry |
...
**`venv_resolve.py`**: `resolve_venv` :82 · `venv_python` :115 · `_is_default_runtime_dir` :156 · `_test_venv` :166 · `_test_venv_detail` :172 · `_create_venv` :246 · `_PythonSelectionPolicy` :290 · `_normalize_selector_architecture` :306 · `_parse_macos_major` :316 · `_python_selection_policy` :330 · `_probe_python_candidate` :402 · `_selection_remedy` :495 · `_find_python` :509 · `_env_marker_status` :635 · `_env_marker_status_detail` :640 · `_remove_mismatched_managed_venv` :726 · `_env_marker_main` :741
