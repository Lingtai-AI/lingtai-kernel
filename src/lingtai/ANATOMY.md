...
| `adapters/project_workspace.py` | Filesystem implementations of the Project Core Ports: fresh `.lingtai` seed creation plus one-level, marker-only, read-only Project inspection. |
| `cli.py` | `lingtai-agent run <dir>` / `lingtai-agent acp --agent-dir <dir>` / `lingtai-agent acp --profile puffo-v0\|puffo-v1 --runtime-id <id>` / `lingtai-agent puffo-v0 provision\|revoke` / `lingtai-agent project create\|inspect ...` / `lingtai-agent check-caps` / `lingtai-agent log ...` / `lingtai-agent maintenance cleanup <target>` entry points; the `run` composition root performs a post-stop hard exit only when existing worker-poison state would otherwise keep the old process alive and block the refresh watcher |
| `cli_project.py` | Inbound composition for `project create` and read-only `project inspect`: create alone loads caller preset/covenant and validates through current readers; inspect composes only its filesystem adapter and deterministic output. Neither starts an Agent. |
...
**`cli_project.py`**: `add_project_parser` · `_request` · `_validate_agent` · `_handle_inspect` · `handle_project_command`
...
**Project ownership:** `cli_project` composes caller inputs, wrapper `load_preset`, current owner readers, and `FilesystemProjectWorkspaceAdapter` for fresh creation. Its inspect branch instead composes only `FilesystemProjectInspectionAdapter`, which reads direct canonical marker shapes without reading content or starting an Agent.
