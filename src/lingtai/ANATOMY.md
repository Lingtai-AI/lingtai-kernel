...
| `cli.py` | `lingtai-agent run <dir>` / `lingtai-agent acp --agent-dir <dir>` / `lingtai-agent acp --profile puffo-v0 --runtime-id <id>` / `lingtai-agent puffo-v0 provision\|revoke` / `lingtai-agent project create ...` / `lingtai-agent project rename ...` / `lingtai-agent check-caps` / `lingtai-agent log ...` / `lingtai-agent maintenance cleanup <target>` entry points; the `run` composition root performs a post-stop hard exit only when existing worker-poison state would otherwise keep the old process alive and block the refresh watcher |
| `cli_project.py` | Inbound composition for fresh `project create` plus narrow POSIX `project rename`: creation composes caller inputs, wrapper preset loading, current-reader validation, and the Project adapter without starting an Agent; rename foreground-invokes the bundled address-change supervisor and treats any nonzero or unstartable terminal outcome as failure. |
...
**`cli_project.py`**: `add_project_parser` · `_request` · `_validate_agent` · `_handle_rename_command` · `handle_project_command`
...
**Project address rename:** `cli_project` invokes the bundled POSIX address-change helper in foreground mode after its required external-writer acknowledgement. The helper retains lifecycle proof, held lease, native no-replace move, limited metadata rebase, and restart evidence; it is not a `kernel.project` fresh-create Port or a generic filesystem operation.
