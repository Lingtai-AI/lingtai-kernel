...
| `agent.py` | **THE key file.** `Agent(BaseAgent)` — layer-2 agent with capability composition, preset swap, persistent MCP plus explicit process-local session-MCP leases, init.json refresh, and default POSIX event-journal + notification-store + agent-presence + workdir-lease + snapshot/source-revision injection for outer callers (it selects the platform lease via `lingtai.adapters.workdir_lease.select_workdir_lease` when `working_dir` is present and no `workdir_lease` was passed, and constructs `PosixAgentPresenceStoreAdapter(working_dir)` when no `agent_presence` was passed — see `kernel/agent_presence/CONTRACT.md`). It selects the refresh-watcher capability through `lingtai.adapters.refresh_watcher.select_refresh_watcher` when no watcher is injected. It also constructs the portable `SystemLifecycleClockAdapter()` when no `lifecycle_clock` was passed (no `working_dir` needed — see `kernel/lifecycle_clock/CONTRACT.md`). Its narrow `resolve_cache_miss_budget()`, `resolve_notification_max_chars()`, and `resolve_runtime_policy()` methods are lazy outer composition hooks from kernel metadata and boot/refresh setup to the System-owned `tools/system/settings.py` resolvers; Core never imports that tool module. Selection and `BaseAgent` construction share one guard that closes the wrapper-owned journal best-effort while preserving the original failure. `mount_session_mcp_stdio` delegates atomic overlay publication to `services/session_mcp.py`, and Agent stop closes those leases after execution quiescence. Mounted MCP tools preserve the server's advertised schema, and stay current with it: `_mount_mcp_tools` publishes a client's catalog under the shared tool-surface lock and registers `_reconcile_mcp_client_tools` as that client's `watch_tools_changed` listener, so a standard `tools/list_changed` (see `services/ANATOMY.md` `_ToolCatalogWatch`) atomically adds/replaces/removes exactly the tools that client owns — handler identity kept for unchanged records, metadata sidecar and routes updated, other clients and native tools untouched, the whole delivery rejected (last good catalog kept) on a malformed record or a name collision, and ignored once the client is discarded or closed. Session-overlay (`services/session_mcp.py`) and daemon task MCPs are not watched. **The shared tool-surface lock (`services/session_mcp._surface_lock`, a reentrant per-agent lock) is the one invariant covering every mutator of `_mcp_clients` / `_mcp_clients_by_tool` / `_mcp_tool_metadata` / `_mcp_tool_collisions`/`_tool_handlers`/`_tool_schemas`, not only reconcile and mount:** `connect_mcp`/`connect_mcp_http` take it for their dedup-check read and their append publication (never across `client.start()`/`client.list_tools()`, which run unlocked); `_discard_mcp_client` — the single close/remove/forget teardown path used by mount rollback, preflight failure, and `_retry_failed_mcps`'s dead-client and disconnected-replacement cleanup alike — and `_forget_mcp_client_tools` both take it internally (reentrant-safe, since mount rollback already calls `_discard_mcp_client` while holding it); and `_retry_failed_mcps` reads its own before/after `_mcp_clients` snapshots under the same lock around the unlocked `connect_mcp*` reconnect attempt. `stop`/refresh teardown (`_close_agent_owned_services_after_quiescence`, `_setup_from_init`) take the identical lock, so a `tools/list_changed` reconciliation can never interleave with any of these — mount, discard, retry cleanup, or full teardown — leaving a torn or dict-corrupted surface. Exact closed LTP-v2 families receive `_reasoning` → public `reasoning` restoration, while stdio/HTTP handlers copy arguments and remove only server-undeclared host-private fields immediately before the provider call. Explicitly declared private fields and ordinary unknown business fields remain untouched. Tool schemas registered here carry package ownership into the inherited BaseAgent inventory renderer; `Agent` no longer duplicates tool-inventory rendering, so package glossaries are appended once in the kernel path. |
...
| `cli.py` | `lingtai-agent run <dir>` / `lingtai-agent acp --agent-dir <dir>` / `lingtai-agent acp --profile puffo-v0\|puffo-v1 --runtime-id <id>` / `lingtai-agent puffo-v0 provision\|revoke` / `lingtai-agent project create ...` / `lingtai-agent check-caps` / `lingtai-agent log ...` / `lingtai-agent maintenance cleanup <target>` entry points; the `run` composition root first refuses any `.change-name-incomplete` marker shape before cleanup/init/Agent construction, and performs a post-stop hard exit only when existing worker-poison state would otherwise keep the old process alive and block the refresh watcher |
...
| `cli_daemon.py` | Thin `lingtai-agent daemon emanate|list|check|reclaim` driver over `services.daemon.DaemonService`; every action requires `--state-root`, native task payloads carry direct preset paths, and there is no Agent/init.json/preview compatibility route |
| `services/daemon.py` | Reusable standalone daemon composition: owns one filesystem state root, reuses `DaemonManager` + `DaemonFamilyDispatcher`, requires direct presets for native LingTai work, and provides mutation-free durable `list`/`check` readback without constructing an Agent |
...
**`services/daemon.py` / `cli_daemon.py`**: `DaemonService` · `_StandaloneDaemonRuntime` · `_ReadOnlyDaemonView` · `DaemonService.emanate/list/check/reclaim`; the CLI contributes only bounded JSON loading, rendering, parser registration, and `handle_daemon_command` over that service.
...
| `<workdir>/.change-name-incomplete` | POSIX Agent-name helper; `cli.run` early gate | Durable transaction fence created before same-parent rename and moved with the workdir. Any shape blocks ordinary launch before signal cleanup/init/Agent construction; the CLI never consumes it. |
| `<workdir>/.mcp_inbox/` | MCPInboxPoller (started at :701) | LICC events from out-of-process MCPs |
| `<workdir>/system/agent_record.json` | `BaseAgent._write_session_stats_record` (kernel) via `kernel.session_stats.build_agent_record`, extended by `Agent._build_agent_record_extra` | The Agent Record — one atomic/versioned/redacted live personal record every Agent (including avatars) publishes, throttled by `LINGTAI_SESSION_STATS_REFRESH_SECONDS`. Its independently versioned `async_work` child is the kernel-owned fixed-600-second daemon+Shell presentation snapshot, produced by the existing single-flight background seam and omitted until a complete snapshot exists. `Agent._build_agent_record_extra` adds `handles`/`integrations` from `services.mcp_registry` (Core itself never imports MCP modules). See `kernel/session_stats/ANATOMY.md` and `CONTRACT.md`. |
