...
| File | Role |
|---|---|
| `__init__.py` | Lazy public API facade — re-exports every name in ``__all__`` on first access via PEP-562 ``__getattr__`` from its canonical source module (``_LAZY_EXPORTS``, ``__init__.py:18-71``). ``import lingtai`` loads only the stdlib and the package version; heavy implementation modules are resolved lazily. |
...
- **AgentConfig hydration:** `_setup_from_init` rebuilds runtime config via `build_agent_config`, overlaying explicit init.json values onto `lingtai.kernel.config.AgentConfig` defaults. `manifest.cache_miss_budget` overlays `AgentConfig.cache_miss_budget` (default 1,000,000). Legacy `max_turns` and `molt_*` manifest values remain deliberately ignored. `manifest.llm.thinking` hydrates verbatim when present, against a **provider-scoped** vocabulary: `thinking_levels_for_llm` (`kernel/config.py`) returns the six Responses values `none`/`minimal`/`low`/`medium`/`high`/`xhigh` for Codex-family providers and explicit custom OpenAI-compatible Responses, and the three K3 coding values `low`/`high`/`max` (`KIMI_THINKING_LEVELS`) for the `kimi-code`/`kimi_code` route (`KIMI_THINKING_PROVIDERS`), so `max` stays rejected on Responses and `none`/`minimal`/`medium`/`xhigh` stay rejected on Kimi. `llm_supports_thinking` is the thin scope predicate over that same resolution and remains the shared schema/preset rule. When omitted, both Codex-family and Kimi Code providers keep the `THINKING_OMITTED` (`"default"`) sentinel so each adapter owns its own omission semantics — Codex applies `reasoning.effort = "xhigh"`, Kimi Code sets no `KIMI_MODEL_THINKING_EFFORT` variable at all — while custom Responses and all other providers keep the legacy cross-provider `"high"` main-session default. `AgentConfig.thinking` additionally defaults to a module-private constructor-omitted sentinel resolved in `__post_init__` (recording `AgentConfig.thinking_omitted`), so a directly constructed Kimi config cannot turn a silent default into an explicit effort its always-thinking default model would reject; `SessionManager._session_thinking` applies the same provider-scoped rule at chat creation/rebuild.
