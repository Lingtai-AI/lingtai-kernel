...
**`init_schema.py`**: `DEPRECATED_TOP_FIELDS` :48 (plain deprecated top-level fields stripped by `strip_deprecated`), `LEGACY_MIGRATED_TOP_FIELDS` :67 (legacy fields removed by version-controlled agent migrations and known only as inactive schema), `validate_init` :177, `TOP_OPTIONAL` :22, `MANIFEST_OPTIONAL` :86, `MANIFEST_LEGACY_IGNORED` :144 (recognized-and-ignored manifest fields — `molt_*`, `stamina`, and `max_turns`, which `build_agent_config` deliberately ignores); `manifest.llm.compact_threshold` is validated as positive int or null in the LLM block, and `manifest.cache_miss_budget` (the soft cache-miss token budget) is validated as a positive int — bool and `<= 0` rejected — with a clear `manifest.cache_miss_budget` error path. The operational knobs `activeness`/`snapshot_interval`/`aed_timeout`/`max_aed_attempts` are typed in `MANIFEST_OPTIONAL` and range-checked (:354, :376) so a malformed value fails at validation instead of detonating at runtime — the schema tables are the single source of truth for what `build_agent_config` honors, cross-checked against the hydrator by `test_hydrator_manifest_keys_are_schema_known` (issue #736).
...
- **AgentConfig hydration:** `_setup_from_init` :1137 rebuilds runtime config via `build_agent_config`, overlaying explicit init.json values onto `lingtai_kernel.config.AgentConfig` defaults. `manifest.cache_miss_budget` overlays `AgentConfig.cache_miss_budget` (default 1,000,000); the operational knobs `activeness`/`snapshot_interval`/`aed_timeout`/`max_aed_attempts` are honored too and are typed + range-checked in `init_schema.MANIFEST_OPTIONAL` so a malformed value fails at validation, not at runtime (issue #736). Every top-level manifest key the hydrator reads must be schema-known — the AST guard `test_hydrator_manifest_keys_are_schema_known` enforces this. Legacy `max_turns` and `molt_*` manifest values remain deliberately ignored (recognized-and-ignored via `MANIFEST_LEGACY_IGNORED`).
