...
- **AgentConfig hydration:** `_setup_from_init` rebuilds runtime config via `build_agent_config`, overlaying explicit init.json values onto `lingtai.kernel.config.AgentConfig` defaults. `manifest.cache_miss_budget` overlays `AgentConfig.cache_miss_budget` (default 1,000,000). Legacy `max_turns` and `molt_*` manifest values remain deliberately ignored. `manifest.llm.thinking` hydrates verbatim when present (schema values `none`/`minimal`/`low`/`medium`/`high`/`xhigh` for Codex/custom Responses, `none`/`high`/`max` for Zhipu-GLM) for Codex-family providers, explicit custom OpenAI-compatible Responses, and `zhipu`/`glm`; `llm_supports_thinking` is the shared schema/preset scope rule and `thinking_levels_for_llm` the shared vocabulary rule. When omitted, Codex-family providers (`THINKING_PROVIDERS`) keep the `"default"` sentinel so the Codex adapter applies its own default (`reasoning.effort = "xhigh"`), and Zhipu-GLM (`ZHIPU_THINKING_PROVIDERS`) likewise keeps the sentinel so the adapter emits no reasoning fields and GLM applies its documented `enabled`/`max` default, while custom Responses and all other providers keep the legacy cross-provider `"high"` main-session default.
