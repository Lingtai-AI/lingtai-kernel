...
- `session.py` — `SessionManager`. LLM session lifecycle, token bookkeeping, chat history persistence, AED (auto-error-recovery) retry path. `send()` owns prompt/tool refresh, health checks, LLM dispatch, usage accounting, and the fail-open cache-audit hook; notification delivery stays in `BaseAgent._sync_notifications` so session sends never mutate unrelated tool-result content.
- `cache_audit.py` — fail-open JSONL writer for normalized LLM usage/cache observations. `SessionManager` constructs it when given a `logs/cache_audit.jsonl` path; construction/write failures must never block agent startup or LLM calls.
