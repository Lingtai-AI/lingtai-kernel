...
def _reasoning_observability_fields(chat: Any) -> dict[str, str]:
    ...
    names with provider-specific values; the Kimi Code route is the first.
...
class SessionManager:
    ...
    def _session_thinking(self) -> str:
        ...
        The Kimi Code route owns its own omission: a constructor-omitted value
        must stay the internal ``THINKING_OMITTED`` sentinel (the adapter then
        emits no ``KIMI_MODEL_THINKING_EFFORT`` environment variable at all)
        instead of being promoted to the legacy cross-provider ``"high"``
        default, which would silently hand every existing Kimi agent an
        explicit effort request — and the always-thinking default model
        ``kimi-for-coding`` rejects one. An explicit value passes through
        unchanged, including a falsey one, which the Kimi contract rejects
        loudly at ``create_chat`` rather than papering over here. Every other
        provider keeps the historical ``or "high"``.
        """
        thinking = self._config.thinking
        if self._effective_provider() in KIMI_THINKING_PROVIDERS:
