...
- Query helpers accept read-only `SELECT`/CTE/`EXPLAIN` statements and open with SQLite `mode=ro` plus `PRAGMA query_only=ON`, so CLI inspection never writes database content and a long-lived reader still sees later writer commits; on a live store SQLite may materialize `-wal`/`-shm` sidecars as a read side effect. A schema probe runs before the reader is accepted, and a genuinely read-only store (archived logs, read-only mount) retries once with `immutable=1`, but only when no `-wal` sidecar exists, so immutable reads can never hide uncheckpointed rows (`services/logging.py:208-248`, `services/logging.py:599-613`).
