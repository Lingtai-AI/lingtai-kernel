...
def load_preset(
    ...
    *,
    ...
    if "thinking" in llm:
        # Provider-scoped vocabulary — see ``thinking_levels_for_llm``. Kimi
        # Code takes the K3 coding service's three native effort levels; Codex
        # and custom Responses keep the six Responses values.
        ...
        if allowed is None:
            raise ValueError(
                ...
                f"({', '.join(THINKING_PROVIDERS)}), the Kimi Code providers "
                f"({', '.join(KIMI_THINKING_PROVIDERS)}) or custom "
                "OpenAI-compatible Responses"
            )
        thinking = llm["thinking"]
        if not isinstance(thinking, str) or thinking not in allowed:
            raise ValueError(
...
def materialize_active_preset(
    ...
    *,
    ...
    ``lingtai.tools.registry.CORE_DEFAULTS`` from the wrapper. It lives in
