...
- `logs/refresh_relaunch.log` as a read-only footprint observation only: never a cleanup candidate, not a protected-section entry
...
- `logs/events.jsonl`, `logs/token_ledger.jsonl`, and `logs/refresh_failed_permanent.json` are protected authoritative or recovery logs. `logs/refresh_relaunch.log` is a footprint-only observation and never a candidate; an unclassified log such as the removed Soul subsystem's leftover `logs/soul_flow.jsonl` is simply not scanned and is never a candidate either. `logs/log.sqlite` is rebuildable and may be reported as a candidate when stale.
