...
- `llm/reasoning.py` — deliberately small provider-neutral construction boundary: immutable `ReasoningRouteKey`/`ReasoningRouteContext`, the opaque `ReasoningWirePatch` protocol, and `ReasoningConstructionResult` (`llm/reasoning.py:15-48`), and the `ReasoningContract` protocol (`llm/reasoning.py:51-59`). It owns no registry or selection policy — the provider factory binds a contract directly — and contains no provider defaults, model catalogue, or wire field path. `ChatSession.reasoning_construction_result()` (`llm/base.py:221-223`) returns `None` by default; provider sessions may return their exact immutable capture. The one current consumer is governed by `src/lingtai/llm/openai/CONTRACT.md`.
