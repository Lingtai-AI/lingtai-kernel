...
related_files:
  ...
  - src/lingtai/kernel/llm/routes.py
...
- `llm/policy.py` — immutable construction-time `ReasoningPolicy`, the neutral one-invocation `ReasoningConstruction` result each route-local mapper returns, and the exact provider-emission evidence used by the kernel's additive `llm_call` record; it carries no provider defaults or live state. The stored result is deeply frozen (nested JSON snapshots, no mutable SDK objects), sites merge byte-identical copies via `materialize_json()`, and evidence paths are verified exactly against the delta under an owner-declared `merge_root` — no prefix guessing.
- `llm/routes.py` — immutable current-wiring route catalogue (`REASONING_ROUTES`) keyed by integration/provider/endpoint-identity/model-predicate/api-compat/wire/CLI-phase; carries admission, baseline, disposition, exact path, and defect facts, and is queryable by those dimensions via `find_reasoning_routes()` (model predicates evaluate mechanically). `llm_supports_thinking` is its manifest-admission projection (re-exported by `kernel/config.py`). Descriptive only — output mapping stays in the provider-local mappers.
...
- `llm/streaming.py` — `StreamingAccumulator`, which gathers streaming text/thought/tool-call deltas and finalizes to `LLMResponse` (`llm/streaming.py:19-63`, `llm/streaming.py:161-197`). Sequential tool-call assembly accepts either deltas or a first non-empty complete terminal argument string via `set_tool_args_if_empty()` (`llm/streaming.py:72-101`); index-keyed deltas remain separate (`llm/streaming.py:102-133`), as do atomic tool calls (`llm/streaming.py:135-139`) and `_finalize_tool()` (`llm/streaming.py:200-214`).
...
- `session.py` captures `ReasoningPolicy` at session construction and reads only an adapter-provided immutable emission record when it adds the optional `llm_call.reasoning` field.
