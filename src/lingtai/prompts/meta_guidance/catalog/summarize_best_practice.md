...
---
Use progressive disclosure for tool results: raw output is for inspection, not routine storage. Prefer a priori `summary=true` before calling `shell`, `file` (read/glob/grep), or `daemon` when you expect bulky output and already know the facts, counts, anchors, or conclusion you need; encode that retention contract in `reasoning` so raw bulk never enters context, and keep noisy or bulky work out of main context by delegating it to daemons before it lands here. Treat a posteriori `context(action="summarize")` as a last resort, not routine cleanup — use it only when context is close to overflowing and a molt is unsuitable; batch already-digested results when convenient. Follow any adapter/provider static rules in resident `meta_guidance` in addition to these general rules. Summarize is a mini molt for consumed tool results; if you have already decided to molt, skip pre-molt summarize and molt instead. Do not molt automatically merely because the current task is complete — molt only when context pressure (≥85%), summarize plus automatic reconstruction still cannot bring context below `0.75 * context_window`, the human explicitly asks for a reset, or conversation confusion makes the fresh briefing worth the cost. See `reference/summarize-manual/SKILL.md` (via `context-manual`) for the exact urgent/idle cadences, the `summary_effect` savings fields and lesson-deposit guidance, delayed-reconstruction mechanics, and `tool_call_id` recovery.
