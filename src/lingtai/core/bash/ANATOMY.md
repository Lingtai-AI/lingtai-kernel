...
**Windows test coverage — simulated vs. native.** Two layers keep the Windows strategy honest. `tests/test_bash_windows.py` runs everywhere: it *simulates* a native-Windows host by monkeypatching `_supports_killpg` to `False` and fakes `subprocess`, so the argv/`shell`/`creationflags`/`taskkill` decisions are pinned without a real shell. `tests/test_windows_native_bash_smoke.py` is `win32`-only (skipped on POSIX): it spawns real `powershell.exe` through `BashManager` to exercise sync capture, the async job lifecycle (run → poll → done), cancel of a live sleep, and the `working_dir` sandbox on real Windows separators. Because there is no local Windows runner, the native smoke runs in GitHub Actions — `.github/workflows/windows-smoke.yml` (`workflow_dispatch` + path-scoped `pull_request`) installs `pytest` + `pyyaml`, installs editable package metadata with `pip install -e . --no-deps` to satisfy `lingtai.__version__` without pulling the full runtime/Rust sidecar dependency surface, and runs both files on `windows-latest`. It is a smoke workflow, distinct from the wheel-building `wheels.yml`.
