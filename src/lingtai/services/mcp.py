...
#: Curated messaging-channel families whose full LTP-v2 schema stays out of
#: the provider payload until first use or an inbound notification. This is
#: the wrapper's declarative channel selection at its MCP mount boundary
#: (``Agent._mount_mcp_tools``); Core only knows "a tool with a stub".
DEFERRED_DISCLOSURE_TOOLS = frozenset({"telegram", "feishu", "wechat"})

# Reserved LTP-v2 action every LingTai-owned family offers; the stub's only
# callable action. Spelled here (not imported) to keep this module free of a
# tools-package dependency.
_MANUAL_ACTION = "manual"
...
def deferred_tool_stub(name: str, schema: Any, description: str):
    """Compact manual-only stand-in for a deferred-disclosure channel tool.

    Returns a ``FunctionSchema`` the tool surface advertises instead of the
    full family until the tool is disclosed, or ``None`` when ``name`` is not a
    deferred channel or the advertised schema is not a strict LTP-v2 family
    offering ``manual`` (then nothing can be collapsed safely and the full
    schema stays resident as before). The stub keeps the family's closed
    envelope — required ``action``/``input``/``reasoning`` — restricted to the
    ``manual`` branch, and names the actions that appear after disclosure.
    """
    from lingtai.kernel.llm import FunctionSchema

    if name not in DEFERRED_DISCLOSURE_TOOLS or not is_strict_ltp_v2_family_schema(schema):
        return None
    properties = schema["properties"]
    actions = properties.get("action", {}).get("enum")
    if not isinstance(actions, list) or _MANUAL_ACTION not in actions:
        return None
    deferred_actions = ", ".join(str(a) for a in actions if a != _MANUAL_ACTION)
    identity = _first_sentence(description)
    stub_description = (
        f"{identity} Compact stand-in: the full {name} tool is not loaded yet. "
        f"Call action='{_MANUAL_ACTION}' with input {{}} to read its manual; the "
        f"complete schema (actions: {deferred_actions}) is disclosed on your "
        f"next round. An inbound {name} notification discloses it automatically."
    ).strip()
    parameters = {
        "type": "object",
        "properties": {
            "action": {
                ...
                "enum": [_MANUAL_ACTION],
                "description": (
                    f"Only '{_MANUAL_ACTION}' is callable until the {name} tool is "
                    "disclosed; it returns the usage manual and reveals the full "
                    "action schema on the next round."
                ),
            },
            "input": {
                "type": "object",
                ...
                "description": f"Always an empty object for {_MANUAL_ACTION}.",
            },
            "reasoning": deepcopy(properties["reasoning"]),
        },
        "required": ["action", "input", "reasoning"],
        "additionalProperties": False,
    }
    return FunctionSchema(name=name, description=stub_description, parameters=parameters)
