...
| `mcp.py` | — | `MCPClient` (stdio) + `HTTPMCPClient` (streamable HTTP) — async-to-sync bridges over the official MCP Python SDK v2 `mcp.Client`, with shared schema-aware host-private argument preparation (`prepare_mcp_tool_arguments`), tool-record adaptation, rich-result preservation, and the legacy structured-result projection. Both inherit `_ToolCatalogWatch`, the standard dynamic-catalog tracker: it passes the SDK `message_handler` (direct `notifications/tools/list_changed` on pre-2026-07-28 sessions), feature-detects `Client.listen` and opens a `tools_list_changed` stream when the server advertises `tools.listChanged`, coalesces change signals into one complete paginated refetch (`cache_mode="refresh"`) on the client's own loop, and hands each newer catalog generation to the registered `watch_tools_changed` listener on a single-worker delivery thread — never on the loop. Listener readiness is part of connection readiness: `_async_connect` awaits `_arm_catalog_listen_and_wait` (bounded by `_CATALOG_LISTEN_ACK_TIMEOUT`) before `_ready.set()`, so a change announced by the very first tool call after mount cannot be published to no subscriber. `close()` cancels the listen/refetch tasks and stops delivery; `restart()` re-arms and refetches once. |
