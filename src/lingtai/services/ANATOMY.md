...
| `mcp.py` | 1130 | `MCPClient` (stdio) + `HTTPMCPClient` (streamable HTTP) — async-to-sync bridges over the official MCP Python SDK v2 `mcp.Client`, with shared schema-aware host-private argument preparation (`mcp.py:37-165`), the wrapper's declarative deferred-disclosure selection for the curated messaging channels — `DEFERRED_DISCLOSURE_TOOLS` plus `deferred_tool_stub`, which collapses a strict LTP-v2 family offering `manual` to a manual-only stand-in `FunctionSchema` for `Agent._mount_mcp_tools` (`mcp.py:65-131`) — tool-record adaptation, rich-result preservation, and the legacy structured-result projection |
...
| `mcp_inbox.py` | — | LICC v1 filesystem inbox poller plus Core projection; in-process publication receives the agent and uses its injected Notification Store while the external inbox path/envelope stays unchanged (`src/lingtai/services/mcp_inbox.py:384-406`). `notification_channel(mcp_name)` (`src/lingtai/services/mcp_inbox.py:65-74`) is the one definition of the `mcp.<name>` channel that `_dispatch_summary` publishes under and that `Agent._mount_mcp_tools` declares as a deferred tool's `disclosure_sources`. |
