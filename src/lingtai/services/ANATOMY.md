...
- **Stale-resource recovery (issues #104 stdio, #740 HTTP):** both `MCPClient` (stdio) and `HTTPMCPClient` (HTTP/SSE) detect a dead transport in `call_tool` and recover. `_format_exception` renders `ClassName: message` (class-only when `str(e)` is empty) so an empty `ClosedResourceError` never surfaces as a blank `{"status":"error","message":""}`. `_is_stale_resource_error` flags closed/broken transports by class name + message substrings. On a stale error `call_tool` calls `restart()` (which `close()`s, clears `_ready`/`_error`, resets `_closed`/`_session`/`_loop`/`_thread`/streams/`*_cm` so `start()` cannot lie) and retries **once**; a failed retry returns a helpful error naming the class and the retry failure. Non-stale errors surface the class name without churning the connection, and all outcomes (including failures) are recorded in `_activity_log`. `HTTPMCPClient` reuses `MCPClient._format_exception` / `MCPClient._is_stale_resource_error` (also for its connect error). Both clients therefore convert transport failures into a dict rather than raising. Tests: `tests/test_mcp_closed_resource_restart.py`.
