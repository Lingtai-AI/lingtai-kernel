...
| `mcp.py` | — | `MCPClient` (stdio) + `HTTPMCPClient` (streamable HTTP) — async-to-sync bridges over the official MCP Python SDK v2 `mcp.Client`, with shared schema-aware host-private argument preparation (`prepare_mcp_tool_arguments`), tool-record adaptation, rich-result preservation, and the legacy structured-result projection. Both share `_ToolCatalogWatch`: the standard `tools/list_changed` signal (`subscriptions/listen` on 2026-07-28, the direct notification earlier) triggers a full refetch on the client loop and hands the records to one registered listener on a delivery thread; the listen route is resolved before the client reports ready |
