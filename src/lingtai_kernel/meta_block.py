...
# Feishu mirrors the Telegram persistent lane at
# `_meta.notification_persistent.mcp.feishu` (same `mcp` namespace level as the
# ephemeral `notifications.mcp.feishu` hook).
...
# The Feishu producer's structured preview carries the last 10 conversation
# messages (FeishuManager._build_conversation_preview_and_metadata), so the
# seed/delta boundary matches that window rather than Telegram's 20.
...
NOTIFICATION_PERSISTENT_FEISHU_REFERENCED_COMMENT = (
    "This is the full Feishu message referenced by the current reply; "
    "included because it is not present in messages."
)
...
def build_meta_readme() -> dict:
    ...
    return {
        ...
        NOTIFICATION_PERSISTENT_KEY: (
            "Sparse communication-context lane, currently email plus the "
            "Telegram and Feishu IM channels. Email context lives under "
            f"{NOTIFICATION_PERSISTENT_EMAIL_PATH}; IM channels carry "
            ...
            "pointing to the prior block of the same channel (and an optional "
            "human-readable comment). IM blocks also carry concise English "
            "machine comments: `.context_comment` (a seed block's historical "
            "id range plus the current/new message id), `.burst_comment` "
            "(multiple new incoming messages arrived together — one burst), "
            ...
            "to the producer's read action — telegram.read / feishu.read — "
            "for exact full producer state). This is the durable model-visible "
            "home for conversation context and routing details — the ephemeral "
            "_meta.notifications.<channel> lane is only a short high-attention "
            "hook carrying stable ids (message_ids / email_ids), not a holder "
            "for message text, sender/subject, routing refs, counts, or "
            "content-location pointers. It is not a "
...
def _im_preview_list(notification_payload: dict, channel_key: str) -> list[dict]:
    """Return one IM channel's notification preview entries from the payload.

    ``channel_key`` is the transient notifications key, e.g. ``mcp.telegram``
    or ``mcp.feishu``. The preview shape is the generic LICC preview built by
    ``inbox._consume_event``, so this helper is channel-agnostic.
    """
    notifications = notification_payload.get(NOTIFICATIONS_KEY)
    if not isinstance(notifications, dict):
        return []
    channel = notifications.get(channel_key)
...
def _im_fallback_message_from_preview(preview: dict) -> dict | None:
    ...
    Reads only the generic LICC preview keys (``preview``, ``message_ref``,
    ``from``, ``subject``, ``conversation_ref``, ``platform``), so it works for
    any curated IM producer that has not (yet) attached structured
    ``recent_messages`` / ``latest_incoming`` metadata.
    """
...
def _im_persistent_events_from_notifications(
    notification_payload: dict, channel_key: str
) -> list[dict]:
    """Extract IM event/routing hooks from notification preview metadata."""
    events: list[dict] = []
    for preview in _im_preview_list(notification_payload, channel_key):
...
def _im_notification_event_count(notification_payload: dict, channel_key: str) -> int:
...
def _im_persistent_messages_from_notifications(
    ...
    fields.  If an older or degraded notification has only the bounded body
    preview, move that preview into the persistent lane as a fallback message
    so the transient notification never carries conversation content.
    """
    by_id: dict[str, dict] = {}
    order: list[str] = []
    for preview in _im_preview_list(notification_payload, channel_key):
...
def _im_referenced_messages_from_notifications(
    ...
    Curated IM producers attach the full referenced message under
    ``referenced_messages`` when the current reply targets a message outside
    the recent-context window. De-duplicate by compound ID, preserving
    first-seen order.
    """
    by_id: dict[str, dict] = {}
    order: list[str] = []
...
def _im_display_message_number(compound_id: object) -> str:
    """Return a robust human-facing message number from an IM compound ID.

    Compound IDs are ``account:chat:message``; the trailing segment is the
    producer's message id.  Falls back to the raw value when the shape is
...
def _im_range_context_comment(messages: list[dict], *, label: str) -> str | None:
    ...
    context using robust ids drawn from the compound ids.  ``label`` is the
    human-facing channel name (``Telegram`` / ``Feishu``).  Returns ``None``
    when there is no historical range to describe (e.g. a single-message
    block).
    """
    if len(messages) < 2:
        return None
    ...
    return (
        f"{span} from the recent {label} conversation. "
        f"The current/new message is {current_num}."
    )


def _annotate_im_message(
    message: dict,
    *,
    ...
    if annotated.get("direction") == "outgoing":
        hints.append(self_outgoing_comment)
    if annotated.get("text_truncated"):
        hints.append(truncated_comment)
    if hints:
...
# Per-channel wiring for the IM persistent-context lane.  Each entry drives the
# generic `_build_im_notification_persistent_payload` /
# `_sanitize_im_notification_after_persistent` /
# `record_notification_persistent_delivery` machinery so every curated IM
# producer gets the same PR #705/#707 attention/context split without
# duplicating the builder per channel.  The dict key is the channel name under
# `_meta.notification_persistent.mcp.<channel>`.
_IM_PERSISTENT_CHANNEL_SPECS: dict[str, dict] = {
    NOTIFICATION_PERSISTENT_TELEGRAM_CHANNEL: {
        "channel_key": "mcp.telegram",
        "label": "Telegram",
        ...
        "ids_attr": "_notification_persistent_telegram_message_ids",
        "tool_id_attr": "_notification_persistent_telegram_last_tool_id",
    ...
    NOTIFICATION_PERSISTENT_FEISHU_CHANNEL: {
        "channel_key": "mcp.feishu",
        "label": "Feishu",
        ...
        "ids_attr": "_notification_persistent_feishu_message_ids",
        "tool_id_attr": "_notification_persistent_feishu_last_tool_id",
        ...
        "referenced_comment": NOTIFICATION_PERSISTENT_FEISHU_REFERENCED_COMMENT,
    },
}


def _build_im_notification_persistent_payload(
    agent, notification_payload: dict, spec: dict
) -> dict | None:
    """Build the minimal `_meta.notification_persistent` payload for one IM channel.

    First delivery after startup/molt (or when fewer than the channel's minimum
    ...
    carry messages whose compound IDs have not been delivered yet, plus a short
    comment pointing to the previous persistent block.
    """
    channel_key = spec["channel_key"]
    candidates = _im_persistent_messages_from_notifications(
        notification_payload, channel_key
    )
    events = _im_persistent_events_from_notifications(notification_payload, channel_key)
    if not candidates and not events:
        return None

    delivered = getattr(agent, spec["ids_attr"], [])
    if not isinstance(delivered, (list, tuple, set)):
        delivered = []
    delivered_ids = {msg_id for msg_id in delivered if isinstance(msg_id, str)}
    previous_tool_id = getattr(agent, spec["tool_id_attr"], None)
    ...
    # block of this channel to link to.
    min_context = spec["min_context"]
    has_recent_context = (
        has_previous_block and len(delivered_ids) >= min_context
    )
    ...
    if candidates and has_recent_context:
        ...
        messages = candidates[-min_context:]
        is_seed_block = True
    else:
        messages = []
    ...
    for message in messages:
        annotated_messages.append(
            _annotate_im_message(
                message,
                self_outgoing_comment=spec["self_outgoing_comment"],
                truncated_comment=spec["truncated_comment"],
            )
        )
    ...
    channel_payload: dict = {"messages": messages}
    ...
    if is_seed_block:
        range_comment = _im_range_context_comment(messages, label=spec["label"])
        if range_comment:
            channel_payload["context_comment"] = range_comment

    # Burst: multiple genuinely new incoming messages arrived together.  Seed
    # blocks carry historical recent-window context, so do not count those
    # historical messages as a burst unless the producer's notification count
    # says multiple new events triggered this block.
    event_count = _im_notification_event_count(notification_payload, channel_key)
    if (not is_seed_block and new_incoming >= 2) or event_count >= 2:
        channel_payload["burst_comment"] = spec["burst_comment"]
    ...
    for ref in referenced:
        if ref.get("id") in present_ids:
            continue
        annotated = _annotate_im_message(
            ref,
            self_outgoing_comment=spec["self_outgoing_comment"],
            truncated_comment=spec["truncated_comment"],
        )
        existing = annotated.get("comment")
        if isinstance(existing, str) and existing:
            annotated["comment"] = f"{spec['referenced_comment']} {existing}"
        else:
            annotated["comment"] = spec["referenced_comment"]
        referenced_out.append(annotated)
    if referenced_out:
        channel_payload["referenced_messages"] = referenced_out

    if events:
        channel_payload["events"] = events

    # Every IM persistent block carries an explicit hook to the previous block
    # of the same channel (Jason #6148). The first block after startup/molt has
    # no predecessor: it is marked `is_first_block: true` with
    # `tool_result_id: null`. Later blocks point `tool_result_id` at the prior
    # tool result id.
    ...
    if is_first_block:
        ...
        previous_block["comment"] = (
            f"For earlier {spec['label']} context, see tool result "
            f"{previous_tool_id} at {spec['path']}."
        )
    channel_payload["previous_block"] = previous_block
...
def build_notification_persistent_payload(agent, notification_payload: dict) -> dict | None:
    ...
    for channel_name, spec in _IM_PERSISTENT_CHANNEL_SPECS.items():
        channel_payload = _build_im_notification_persistent_payload(
            agent, notification_payload, spec
        )
...
def record_notification_persistent_delivery(
    ...
    *,
    ...
    for channel_name, spec in _IM_PERSISTENT_CHANNEL_SPECS.items():
        ...
        existing = getattr(agent, spec["ids_attr"], [])
        ...
        for message in messages:
            ...
            if isinstance(msg_id, str) and msg_id and msg_id not in seen:
                ...
                seen.add(msg_id)
        if len(existing) > spec["seen_limit"]:
            existing = existing[-spec["seen_limit"]:]
        try:
            setattr(agent, spec["ids_attr"], existing)
            if tool_call_id:
                setattr(agent, spec["tool_id_attr"], tool_call_id)
        except Exception:
            pass


def _im_notification_message_ids(notification_payload: dict, channel_key: str) -> list[str]:
    ...
    notifications = notification_payload.get(NOTIFICATIONS_KEY)
    channel = notifications.get(channel_key) if isinstance(notifications, dict) else None
...
def _sanitize_im_notification_after_persistent(
    notification_payload: dict, spec: dict
) -> None:
    """Reduce one IM channel's ephemeral lane to a minimal event identity hook.

    Message text, structured context, routing hooks, sender/subject, platform,
    counts, and summaries live under the channel's
    ``_meta.notification_persistent.mcp.<channel>`` block.  The transient
    ...
    No-op when there is no notification for the channel. Safe to call
    unconditionally.
    """
    notifications = notification_payload.get(NOTIFICATIONS_KEY)
    if not isinstance(notifications, dict):
        return
    channel = notifications.get(spec["channel_key"])
    if not isinstance(channel, dict):
        return

    minimal_data: dict = {
        "message_ids": _im_notification_message_ids(
            notification_payload, spec["channel_key"]
        )
    }
    ...
    channel["header"] = f"{spec['label']} event"
    channel["data"] = minimal_data
    channel["instructions"] = (
        f"High-attention {spec['label']} hook: use notification_persistent for "
...
def sanitize_telegram_notification_after_persistent(notification_payload: dict) -> None:
    ...
    _sanitize_im_notification_after_persistent(
        ...
        _IM_PERSISTENT_CHANNEL_SPECS[NOTIFICATION_PERSISTENT_TELEGRAM_CHANNEL],
    )
...
def sanitize_feishu_notification_after_persistent(notification_payload: dict) -> None:
    ...
    _sanitize_im_notification_after_persistent(
        notification_payload,
        _IM_PERSISTENT_CHANNEL_SPECS[NOTIFICATION_PERSISTENT_FEISHU_CHANNEL],
    )
...
def attach_active_notifications(
    ...
    *,
    ...
    # Move (not duplicate): Telegram/Feishu/email durable fields are always
    # stripped from the model-visible ephemeral lane, even when every message id
    # was already delivered and no new persistent block is emitted this round.
