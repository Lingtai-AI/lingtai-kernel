...
4. Otherwise, dispatch on agent state:
   - **IDLE** — `_inject_notification_pair` splices the pair, **then** posts `MSG_REQUEST(content=None)` and `_wake_nap("notification_sync")`. IDLE is "blocked on `inbox.get()`," so without a wake the loop sits forever and the pair never reaches the LLM. The wake message is an empty `MSG_REQUEST` (not `MSG_TC_WAKE`) so it routes to `_handle_request`, which prepends meta and drives a real LLM turn — the synthesized notification pair already at the wire tail is observed by the model. `MSG_TC_WAKE` would route to the dormant `_handle_tc_wake` and no-op (`tc_inbox.drain()` is always empty post-redesign), which is the bug fixed in the empty-turn wake patch — the symptom was the agent transitioning IDLE→ACTIVE→IDLE in milliseconds without ever calling the LLM, leaving the spliced pair unread forever.
   ...
   - **ASLEEP** — clear `_asleep` and `_cancel_event`, transition `IDLE` (reason `notification_arrival`), `_reset_uptime`, then proceed exactly like the IDLE branch (inject pair + post `MSG_REQUEST(None)`). This is the canonical notification-driven wake.
