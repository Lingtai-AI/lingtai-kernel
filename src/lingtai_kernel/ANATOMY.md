...
- `session.py` — `SessionManager`. LLM session lifecycle, token bookkeeping, chat history persistence, pre-send context safety, and AED (auto-error-recovery) retry path. `send()` owns prompt/tool refresh, health checks, the hard context gate (estimate current wire plus pending message, run one deterministic compaction/forced context reset through injected callbacks when possible, and otherwise return a local terminal block before `llm_call`), LLM dispatch, and usage accounting; notification delivery stays in `BaseAgent._sync_notifications` so session sends never mutate unrelated tool-result content except for deliberate completed-tool-result commits when the hard gate blocks after local tools already ran.
