...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning stripping, timing, error capture, and the privileged reserved `secondary` communication side channel: `_validate_secondary` enforces the human-reply whitelist/action policy and recursive-key ban; `_execute_secondary` rejects excluded primary tools at runtime, then dispatches valid communication calls mechanically before the primary, bypassing LoopGuard for the secondary itself while never blocking the primary; `_execute_single` pops `secondary` before the primary LoopGuard key and reports the outcome under the top-level `_secondary` field; `_execute_parallel` runs all allowed secondaries in a sequential pre-pass before worker threads to avoid concurrent human-message fan-out.
- `secondary_tools.py` — policy and schema helpers for reserved nested `secondary` calls: human-reply communication whitelist, action whitelist, primary-tool eligibility exclusions, and the injected JSON-schema property (`secondary_tools.py:15-87`).
