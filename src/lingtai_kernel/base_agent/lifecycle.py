...
def _perform_refresh(
    ...
    *,
    ...
    relaunch_script = (
        ...
        "        return {}\n"
        # Platform-aware process spawn/probe/terminate. On POSIX these resolve to
        # the historical behavior (start_new_session, os.kill(pid,0), SIGTERM/
        # SIGKILL). On native Windows they use CREATE_NEW_PROCESS_GROUP,
        # OpenProcess/tasklist-style liveness, and taskkill. The fallback is
        # deliberately platform-aware too: if the kernel import is unavailable
        # in the child process, the watcher still must not pass POSIX-only
        # kwargs or signals to native Windows.\n"
        "try:\n"
        "    from lingtai_kernel.process_control import (\n"
        "        detached_process_kwargs as _detached_process_kwargs,\n"
        "        pid_is_alive as _pid_is_alive,\n"
        "        terminate_pid as _terminate_pid,\n"
        "    )\n"
        "except Exception:\n"
        "    def _fallback_posix_signals():\n"
        "        return hasattr(os, 'killpg')\n"
        "    def _detached_process_kwargs():\n"
        "        if _fallback_posix_signals():\n"
        ...
        "            return True\n"
        "        try:\n"
        "            completed = subprocess.run(['tasklist', '/FI', f'PID eq {pid}'],\n"
        "                stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=5.0)\n"
        "        except Exception:\n"
        "            return None\n"
        "        if completed.returncode != 0:\n"
        "            return None\n"
        "        output = (completed.stdout or b'').decode('utf-8', 'replace').lower()\n"
        "        if 'no tasks' in output or 'not found' in output:\n"
        "            return False\n"
        "        return str(pid) in output\n"
        "    def _terminate_pid(pid, force=False, tree=False, timeout=5.0):\n"
        "        if _fallback_posix_signals():\n"
        "            try:\n"
        ...
        "        if completed.returncode == 0:\n"
        "            return True\n"
        "        err = (completed.stderr or b'').decode('utf-8', 'replace').lower()\n"
        "        return 'not found' in err or 'not running' in err\n"
        ...
        "        return False\n"
        # _pid_is_alive: True alive, False gone, None indeterminate. Only bail on
        # a definite 'gone'; on alive/indeterminate fall through to the cmdline
        # match (the authoritative same-agent check).\n"
        ...
        # Platform-aware graceful terminate (SIGTERM / taskkill without /F).\n"
        "    if not _terminate_pid(pid, tree=True):\n"
        "        log('refresh_watcher_stale_duplicate_term_error', attempt=attempt,\n"
        "            pid=pid, error='terminate_failed')\n"
        "        failure_state['last_cleanup_result'] = 'sigterm_error'\n"
        "        failure_state['last_cleanup_error'] = 'terminate_failed'\n"
        "        return False\n"
        ...
        "            return True\n"
        "        time.sleep(0.2)\n"
        # Escalate to a forced kill (SIGKILL / taskkill /F /T).\n"
        "    if _terminate_pid(pid, force=True, tree=True):\n"
        ...
        "        pid=pid, error='force_kill_failed')\n"
        "    failure_state['last_cleanup_result'] = 'sigkill_error'\n"
        "    failure_state['last_cleanup_error'] = 'force_kill_failed'\n"
        "    return False\n"
        ...
        "                stderr=serr, **_detached_process_kwargs())\n"
        "    except Exception as e:\n"
    ...
    from ..process_control import detached_process_kwargs
    subprocess.Popen(
