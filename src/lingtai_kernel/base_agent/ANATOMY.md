...
- `base_agent/soul_flow.py` — Soul orchestration: timer, drain, consultation fire, persistence (325 lines). `_start_soul_timer` (`base_agent/soul_flow.py:12-29`), `_cancel_soul_timer` (`base_agent/soul_flow.py:31-36`), `_soul_whisper` (`base_agent/soul_flow.py:38-58`), `_drain_tc_inbox` (`base_agent/soul_flow.py:61-94`), `_persist_soul_entry` (`base_agent/soul_flow.py:96-113`), `_append_soul_flow_record` (`base_agent/soul_flow.py:115-121`), `_run_inquiry` (`base_agent/soul_flow.py:123-135`), `_flatten_v3_for_pair` (`base_agent/soul_flow.py:137-166`), `_run_consultation_fire` (`base_agent/soul_flow.py:168-288`), `_rehydrate_appendix_tracking` (`base_agent/soul_flow.py:290-325`).
...
- `__init__.py` imports from all submodules lazily (inside method bodies) to avoid circular imports at load time.
- `lifecycle.py` → `soul_flow.py` (via `agent._start_soul_timer()`, `agent._cancel_soul_timer()`, `agent._run_inquiry()`).
- `turn.py` → `soul_flow.py` (via `agent._drain_tc_inbox()`, `agent._cancel_soul_timer()`).
...
- `soul_flow.py` imports `from ..state import AgentState`, `from ..message import _make_message, MSG_TC_WAKE`.
...
- `soul_flow.py` writes `logs/soul_flow.jsonl` (`base_agent/soul_flow.py:115-121`), writes `logs/soul_inquiry.jsonl` (`base_agent/soul_flow.py:96-113`), enqueues on `tc_inbox`.
