...
- `base_agent/__init__.py` — BaseAgent class definition (~1027 lines after `f46b346` mid-turn drain). Module-level helpers: `_format_stamina` (`base_agent/__init__.py:54`), `_build_identity_section` (`base_agent/__init__.py:65`). Class definition starts at `base_agent/__init__.py:150`; constructor at `base_agent/__init__.py:176`. Cluster pointers: `_wire_intrinsics` (`base_agent/__init__.py:409`), state machine (`_set_state` at `base_agent/__init__.py:513`, `_wake_nap` at `base_agent/__init__.py:543`, `_log` at `base_agent/__init__.py:548`), session persistence (`get_token_usage` at `base_agent/__init__.py:817`, `get_chat_state`/`restore_chat`/`restore_token_state` at `base_agent/__init__.py:829-837`, `_save_chat_history` at `base_agent/__init__.py:841`), subclass hooks (`_pre_request` at `base_agent/__init__.py:906`, `_post_request` at `base_agent/__init__.py:913`, `_on_tool_result_hook` at `base_agent/__init__.py:919`). **tc_inbox plumbing**: `_drain_tc_inbox` (`base_agent/__init__.py:603`) — main drain entry, idempotently installs the pre-request hook; `_install_drain_hook` (`base_agent/__init__.py:634`) — wires `pre_request_hook` on the active `ChatSession`; `_drain_tc_inbox_for_hook` (`base_agent/__init__.py:693`) — non-installing drain used by the hook to avoid recursion. See root `ANATOMY.md` "Drain points" for the three-trigger story. Plus 46 pass-through stubs to submodules (~92 lines of boilerplate).
...
- `base_agent/messaging.py` — Mail arrival, notifications, outbound messaging (161 lines). `_on_mail_received` (`base_agent/messaging.py:10-18`), `_on_normal_mail` (`base_agent/messaging.py:20-42`), **`_rerender_unread_digest`** (`base_agent/messaging.py:44-97`) — *email-arrival-specific producer that uses replace-in-history single-slot semantics (coalesce=True, source="email.unread")*; **`_enqueue_system_notification`** (`base_agent/messaging.py:100-159`) — *kernel-wide producer hook for surfacing out-of-band events as synthetic tool-call pairs; called by mail bounce, soul, and any new daemon/MCP/scheduler producer; see root `ANATOMY.md` "Involuntary tool-call pairs" for the contract*. `_notify` (`base_agent/messaging.py:161-169`), `_mail` (`base_agent/messaging.py:171-177`), `_send` (`base_agent/messaging.py:179-189`).
...
- **`__init__.py` is ~1027 lines.** This is intentional — the constructor (230 lines), properties, state machine, hooks, cross-cutting infrastructure (`_save_chat_history`, `_log`, the tc_inbox drain trio), and 46 pass-through stubs all live here. The package is not "thin shell + 6 leaves" — it's "1000-line core + 7 specialized helpers." Future readers should not try to extract more from `__init__.py` thinking it should be smaller; the remaining code is genuinely cross-cutting or bound to the class definition.
