...
- `services/mail.py` — message transport.
  ...
  - `send()` resolves peer/absolute addresses, checks `is_agent`/`is_alive`, generates ids through `intrinsics.email._new_mailbox_id`, copies attachments, and writes `message.json` atomically (`services/mail.py:137`, `services/mail.py:171`, `services/mail.py:206-212`).
  - `_dead_letter_inbox_entry()` moves a permanently-corrupt inbox entry directory into `inbox/.dead/` with an `error.json` report and a logged warning, mirroring the MCP inbox poller's dead-letter convention (`services/mail.py:222`, `src/lingtai/core/mcp/inbox.py:399`). Introduced by #732 so malformed mail fails loud instead of vanishing into `_seen`.
  - `listen()` starts a daemon polling thread (`services/mail.py:264`), snapshots existing inbox ids into `_seen` skipping dotted dirs (`services/mail.py:271-276`), scans every 0.5s (`services/mail.py:339`). Phase 1 keeps the three inbox failure classes distinct (`services/mail.py:283-331`): a transient read `OSError` is logged and retried next cycle (never marked `_seen`), a `JSONDecodeError` is dead-lettered, and an `on_message` exception is logged after the entry is already marked seen so a consumer bug never masquerades as lost mail. Phase 2 claims subscribed pseudo-agent outbox messages (`services/mail.py:335-336`, `services/mail.py:344-465`).
  - `stop()` sets the poll stop event and joins the thread (`services/mail.py:547-552`).
...
- **Persistent mail:** `<workdir>/mailbox/{inbox,outbox,sent}/<uuid>/message.json`; optional `attachments/` subdir. `FilesystemMailService.send()` writes recipient inbox payloads atomically (`services/mail.py:206-212`). Permanently-corrupt inbox entries are moved to `inbox/.dead/<uuid>/` with an `error.json` report (`services/mail.py:222`).
...
- **Ephemeral mail:** `_seen` is an in-memory set of delivered UUIDs rebuilt at listen start (`services/mail.py:271-276`); `_poll_thread` is a daemon thread joined by `stop()` (`services/mail.py:547-552`).
...
- Pseudo-agent outbox claiming is optimistic concurrency: pollers copy to inbox, race on outbox→sent rename, and losers delete their speculative copy and clear `_seen` (`services/mail.py:411-448`).
