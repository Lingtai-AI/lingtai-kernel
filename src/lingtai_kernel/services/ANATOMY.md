...
- `services/mail.py` — message transport.
  ...
  - `send()` resolves peer/absolute addresses, checks `is_agent`/`is_alive`, generates ids through `intrinsics.email._new_mailbox_id`, validates all attachment paths up front, then stages the whole message (attachments + `message.json`) in a hidden `.<id>.staging` inbox sibling and publishes it with one atomic `os.replace`, cleaning up staging via `_remove_tree_retry` on any failure so the recipient never observes a partial entry (`services/mail.py:131`, `services/mail.py:165`, `services/mail.py:181-229`).
...
- **Persistent mail:** `<workdir>/mailbox/{inbox,outbox,sent}/<uuid>/message.json`; optional `attachments/` subdir. `FilesystemMailService.send()` publishes each recipient inbox entry via stage-then-rename (`.<uuid>.staging` → atomic `os.replace`), so a partial or failed send never leaves an orphan directory in the recipient's inbox (`services/mail.py:211-229`).
