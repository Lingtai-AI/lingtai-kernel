...
- `soul/consultation.py` — LLM call mechanics for past-self consultations. Constants: `_CONSULTATION_SYSTEM_PROMPT` (`soul/consultation.py:13-31`) — single-pass consultation prompt with cost-of-failure framing; `_CONSULTATION_TOOL_REFUSAL = _CONSULTATION_SYSTEM_PROMPT` (`soul/consultation.py:33`) — refused tool calls return the full system prompt as `ToolResultBlock.content` so the model re-grounds rather than hearing "no"; `_CONSULTATION_MAX_ROUNDS = 3` (`soul/consultation.py:35`); `_DIARY_CUE_TOKEN_CAP = 10_000` (`soul/consultation.py:36`); `_CUE_EVENT_TYPES = ("diary", "thinking")` (`soul/consultation.py:69`) — both event types are mixed into the cue; `_CUE_EVENT_TYPES_BYTES` (`soul/consultation.py:74`) — UTF-8 byte tokens for the substring fast-path; `_REVERSE_READ_CHUNK = 64 * 1024` (`soul/consultation.py:79`). `_send_with_timeout` (`soul/consultation.py:39-66`) wraps LLM calls with a daemon thread. `_iter_lines_reverse` (`soul/consultation.py:82-123`) — tail-first byte-chunked line iterator over a JSONL file; relies on the append-only one-record-per-line invariant and on `\n`'s UTF-8 safety. `_render_current_diary` (`soul/consultation.py:126-194`) builds the time-anchored cue from recent **diary AND thinking** entries; uses `_iter_lines_reverse` with the substring fast-path (`_CUE_EVENT_TYPES_BYTES`) to skip JSON parsing on non-cue lines, stops once `_DIARY_CUE_TOKEN_CAP` is reached. Cost is O(recent cue entries) not O(file size). `_write_soul_tokens` (`soul/consultation.py:197-217`) appends token-ledger entries. `_load_snapshot_interface` (`soul/consultation.py:220-249`) loads a `ChatInterface` from a snapshot file. `_fit_interface_to_window` (`soul/consultation.py:252-338`) tail-trims a `ChatInterface` to a token budget. `_kind_for_source` (`soul/consultation.py:341-345`) maps source labels to prompt kinds. `_build_consultation_cue` (`soul/consultation.py:348-371`) builds localized cue prompts. `_run_consultation` (`soul/consultation.py:374-472`) runs one substrate+spark consultation with refusal loop. `_list_snapshot_paths` (`soul/consultation.py:475-483`) lists snapshot files. `_run_consultation_batch` (`soul/consultation.py:486-557`) orchestrates parallel 1+K consultations. `build_consultation_pair` (`soul/consultation.py:560-592`) builds the synthetic `(ToolCallBlock, ToolResultBlock)` pair.
