...
def test_expand_agent_placeholders_scopes_agent_root(tmp_path):
    # Per-agent root injection: a shared registry template resolves to a root
    # unique to each agent, so agents cannot address each other's namespaces.
    agent, workdir = _mk_agent(tmp_path)  # workdir.name == "agent"
    assert agent._expand_agent_placeholders("/agents/{agent_id}/data") == "/agents/agent/data"
    # {agent_address} is an alias for the stable working-dir name.
    assert agent._expand_agent_placeholders("/agents/{agent_address}/data") == "/agents/agent/data"
