...
1. **Registry matrix** (``REGISTRY_EDGES``) — every registered provider name is
   ...
   route — not through expected prose.
...
3. **Behavior regimes** (``ALL_REGIMES``) — the small set of concrete
   ``ChatSession`` classes with distinct common-input wire behavior. Each builds
   a *real* session with a *mocked transport*; the parametrized tests drive the
   two inputs the kernel ``ChatSession`` ABC declares (``send(str)`` /
