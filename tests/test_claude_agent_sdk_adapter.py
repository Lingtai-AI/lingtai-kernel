"""Tests for the experimental Claude Agent SDK provider.
...
def test_missing_sdk_raises_runtime_error(monkeypatch):
    ...
    session = adapter.create_chat("claude-sonnet-4-20250514", "be helpful")
...
def test_options_disable_tools_single_turn(fake_sdk):
    ...
    assert options.kwargs["model"] == "claude-sonnet-4-20250514"
...
def test_generate_one_shot(fake_sdk):
    ...
    adapter = ClaudeAgentSDKAdapter()
    resp = adapter.generate("claude-sonnet-4-20250514", "do a thing", system_prompt="sys")
