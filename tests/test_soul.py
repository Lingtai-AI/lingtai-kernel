...
class TestSoulSchema:
    ...
    def test_schema_only_exposes_inquiry(self):
        schema = soul.get_schema("en")
        assert schema["properties"]["action"]["enum"] == ["inquiry", "flow"]
        # delay action and delay parameter are gone.
        assert "delay" not in schema["properties"]
