...
def test_mail_inbox_wiring(tmp_path):
    ...
    agent = BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="receiver", working_dir=tmp_path / "test", workdir_lease=make_test_lease())
...
def test_context_stored_opaque(tmp_path):
    ...
    agent = BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="test", working_dir=tmp_path / "test", context=ctx, workdir_lease=make_test_lease())
...
def test_working_dir_required():
    ...
    with pytest.raises(TypeError):
        BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="test", workdir_lease=make_test_lease())
...
def test_config_override(tmp_path):
    ...
    agent = BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="test", working_dir=tmp_path / "test", config=config, workdir_lease=make_test_lease())
...
def test_agent_name_stored(tmp_path):
    ...
    agent = BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="any name 日本語", working_dir=tmp_path / "test", workdir_lease=make_test_lease())
...
def test_working_dir_creates_parents(tmp_path):
    ...
    agent = BaseAgent(intrinsics=_TEST_INTRINSICS, service=make_mock_service(), agent_name="test", working_dir=tmp_path / "deep" / "nested", workdir_lease=make_test_lease())
