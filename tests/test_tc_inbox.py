...
def _make_item(source: str, voice: str = "v", coalesce: bool = False) -> InvoluntaryToolCall:
    ...
    call = ToolCallBlock(id=tc_id, name="demo",
                         args={"action": "flow", "input": {},
                               "reasoning": "involuntary"})
    result = ToolResultBlock(id=tc_id, name="demo", content={"voice": voice})
...
class TestTCInbox:
    ...
    def test_enqueue_coalesce_replaces_existing_with_same_source(self):
        inbox = TCInbox()
        inbox.enqueue(_make_item("demo.flow", voice="first", coalesce=True))
        inbox.enqueue(_make_item("demo.flow", voice="second", coalesce=True))
        assert len(inbox) == 1
        items = inbox.drain()
...
class TestDrainTCInbox:
    ...
    def test_drain_skips_when_chat_none(self, tmp_path):
        ...
        agent._chat = None
        agent._tc_inbox.enqueue(_make_item("demo.flow"))
    ...
    def test_drain_splices_pair_into_wire_chat(self, tmp_path):
        ...
        agent._tc_inbox.enqueue(_make_item("demo.flow", voice="my voice"))
        agent._drain_tc_inbox()
...
class TestReplaceInHistory:
    ...
    appending the new one. Used by single-slot producers."""
    ...
    def test_first_replace_in_history_appends_and_tracks_id(self, tmp_path):
        ...
        item = _make_replace_item("demo.flow", voice="first")
        agent._tc_inbox.enqueue(item)
        agent._drain_tc_inbox()
        # Pair appended; tracker now points at the new call's id.
        assert agent._appendix_ids_by_source.get("demo.flow") == item.call.id
    ...
    def test_second_replace_in_history_evicts_first_pair(self, tmp_path):
        agent, iface = self._make_agent(tmp_path)
        first = _make_replace_item("demo.flow", voice="first")
        ...
        time.sleep(0.002)
        second = _make_replace_item("demo.flow", voice="second")
        ...
        assert agent._appendix_ids_by_source["demo.flow"] == second.call.id

    def test_replace_in_history_only_affects_same_source(self, tmp_path):
        agent, iface = self._make_agent(tmp_path)
        flow_item = _make_replace_item("demo.flow", voice="flow")
        ...
        assert agent._appendix_ids_by_source["demo.flow"] == flow_item.call.id
    ...
    def test_replace_in_history_no_prior_pair_just_appends(self, tmp_path):
        ...
        agent._appendix_ids_by_source["demo.flow"] = "tc_stale_id"
        item = _make_replace_item("demo.flow", voice="fresh")
        agent._tc_inbox.enqueue(item)
        agent._drain_tc_inbox()
        # New pair is appended; stale tracker replaced with the new id.
        assert agent._appendix_ids_by_source["demo.flow"] == item.call.id
    ...
    def test_non_replace_items_skip_history_eviction(self, tmp_path):
        ...
        prior = _make_replace_item("demo.flow", voice="prior")
        ...
        assert agent._appendix_ids_by_source["demo.flow"] == prior.call.id
