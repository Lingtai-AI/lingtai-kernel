...
- **`src/lingtai/cli.py`** — CLI entrypoint (`lingtai run <working_dir>`). Reads `init.json` manifest, creates LLMService + Agent, starts the agent loop. Also `lingtai cpr <dir>` for resuscitating suspended agents.
