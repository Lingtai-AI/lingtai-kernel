...
| `LINGTAI_NUDGE_FOLDER_SIZE_GB` | `5` | Positive number of gigabytes; per-agent working directory | Folder-size Nudge threshold | Each daily folder-size probe; no restart | Missing, zero, negative, or non-numeric values fall back to `5` | `src/lingtai/kernel/nudge/folder_size.py` | Advisory storage hygiene only; never grants deletion or cleanup authority |
